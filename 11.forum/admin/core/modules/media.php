...
function media_invoke()
{
    ...
    $edir  = getoption('uploads_ext') ? getoption('uploads_ext') : getoption('uploads_ext');
    $dfile = cn_path_construct($udir, $path);

    // Remove root identifier    
    $path       = preparation_path($path);
	
print "F_media 163 check path1: $path <br>";
print "F_media 163 check dfile: $dfile <br>";

    // Path detection
    $path       = preg_replace('/[^a-z0-9\/_\\\]/i', '-', $path);
    $root_dir   = cn_path_construct($udir, $path) .DIRECTORY_SEPARATOR;

	print "F_media 163 check root_dir: $root_dir <br>";
	print "F_media 163 check path2: $path <br>";
	print "F_media 163 check udir: $udir <br>";
    ...
    if ($opt === 'inline')
    {
		print "F_media 705 get opt: $opt <br>";
