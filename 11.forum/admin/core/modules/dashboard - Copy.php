...
function dashboard_invoke()
{
	...
	print "F_dashbroad 37(mod) -- 40(opt) : $mod -- $opt <br>";
...
function dashboard_category()
{
	...
	//if(isset($category_id_)) {$category_id = stripslashes($category_id_);$category_id = mysqli_real_escape_string($db,$category_id);}
	//else $category_id =0;
	...
	*/
	//-----------------------------------------
	print "<br><br>F_dashbroad 646 --------------------------------------------- <br>";
	print "F_dashbroad 646 category_id=> ".$category_id." <br>";
	print "F_dashbroad 646 category_name=> ".$category_name." <br>";
	print "F_dashbroad 646 category_memo=> ".$category_memo." <br>";
	print "F_dashbroad 646 category_icon=> ".$category_icon." <br>";
	print "F_dashbroad 646 category_parent=> ".$category_parent." <br>";
	//print "F_dashbroad 646 category_acl=> ".$category_acl." <br>";
	print "F_dashbroad 646 category_description=> ".$category_description." <br>";
	print "F_dashbroad 646 --------------------------------------------- <br><br>";
	//-----------------------------------------
	...
	foreach ($categories_data as $cm=> $hh){
		...
		$categories_call[$_cm]['name'] = $hh['name'];
		$categories_call[$_cm]['memo'] = $hh['memo'];
		$categories_call[$_cm]['icon'] = $hh['icon'];
		$categories_call[$_cm]['acl'] = $hh['acl'];
		$categories_call[$_cm]['parent'] = $hh['parent'];
		$categories_call[$_cm]['level'] = $hh['level'];
		$categories_call[$_cm]['ac'] = $hh['ac'];
		$categories_call[$_cm]['description'] = $hh['description'];
		...
		//foreach ($hh as $h =>$jh)
		//print "F_dashbroad 628 bien ac: $h => $jh <br>";
		//$_category_id   = $id;
		print "F_dashbroad 658 bien categories_call: name=>" . $hh['name'] ."<br>";
		//$_category_memo   = $categories_call[$category_id]['memo'];
		print "F_dashbroad 658 bien categories_call: icon=>" . $hh['icon']."<br>";
		//$_category_acl    = spsep($categories_call[$category_id]['acl']);
		//foreach ($hh['acl'] as $h)
		$keys_acl = explode(',', $hh['acl']);
		$acl_acl=!empty($keys_acl)?join(',', $keys_acl):'';
		print "F_dashbroad 658 bien categories_call: acl=>" .  $hh['acl']."<br>";
		//print "F_dashbroad 658 bien categories_call: acl=>" .  $acl_acl."<br>";
		//$_category_acl=!empty($h)?join(',', $h):'';

		print "F_dashbroad 658 bien categories_call: parent=>" . $hh['parent']."<br>";
		print "F_dashbroad 658 bien categories_call: level=>" .  $hh['level']."<br>";
		//$_category_level   = $categories_call[$category_id]['level'];
		//}
	}
	...
	//--------------------------------------------------------------
	print "F_dashbroad 707 bien category_id max:" . $categories_call['#'] ." <br>";
	//--------------------------------------------------------------
        ...
        if(!empty($category_name))
        {
            ...
            {            
                ...
                foreach ($categories_call as  $value){
					$is_duble=$value['name']==$category_name&&$value['acl']==$cat_acl;
					print "F_dashbroad 729 name=> ".$value['name']."-------acl->".$value['acl']." <br>";
					
					//print "F_dashbroad 729 acl=> ".$c['id']."->".$c['acl']." <br>";
                    if($is_duble) break;
                }           
            ...
            elseif($is_delete)
            {
				//if(is_array($category_id,$categories_call,true)){
				if(array_key_exists($category_id,$categories_call)){
					$_category_ac  = spsep($categories_call[$category_id]['ac']);
					//$_category_ac = explode(',', $categories_call[$category_id]['ac']);
					
					if($_category_ac){
						foreach($_category_ac as $id_val){
							$db->query("DELETE FROM `forums` WHERE id = '".$id_val."';");
							unset($categories_call[$id_val]);
						}
					}
					$db->query("DELETE FROM `forums` WHERE id = '".$category_id."';");
					unset($categories_call[$category_id]);
				}
				
				//echo "delete mysql: $myss <br>";
				//$db->query($myss);
				//$category_id=null;
				//---------------------------------
            ...
            if(!empty($category_id))
            {
                //--------------------------------------------------------------
					...
					$categories_call[$category_id]['name'] = $category_name;
					$categories_call[$category_id]['memo'] = $category_memo;
					$categories_call[$category_id]['icon'] = $category_icon;
					$categories_call[$category_id]['description'] = $category_description;
					$categories_call[$category_id]['acl']  = $cat_acl;
					$categories_call[$category_id]['parent']  = intval( $category_parent);           
					cn_throw_message($message);
					
					//---------------------------------------------------------
					
					print "F_dashbroad 770 -------------start--------------------- <br>";
					print "F_dashbroad 770 id=> $category_id <br>";
					print "F_dashbroad 770 name=> $category_name <br>";
					print "F_dashbroad 770 memo=> $category_memo <br>";
					print "F_dashbroad 770 icon=> $category_icon <br>";
					print "F_dashbroad 770 acl=> $cat_acl <br>";
					print "F_dashbroad 770 parent=> $category_parent <br>";
					print "F_dashbroad 770 ---------------end---------- <br>";
					
					//------------------------------------------------------------------
					if(!$is_duble && !$is_edit)
					{
						$msew ="INSERT INTO `forums`(`id`, `name`, `memo`, `icon`, `acl`, `parent`, `level`, `ac`, `description`) VALUES('"
            ...
            if(!$is_cancel)// || $is_edit || $is_delete)
            {
				...
				foreach ($categories_call as $id =>$hh){
					...
					//$_category_id   = $id;
					$_category_name   = $hh['name'];
					$_category_memo   = $hh['memo'];
					$_category_icon   = $hh['icon'];
					$_category_description   = $hh['description'];
					//$_category_acl    = spsep($categories[$category_id]['acl']);
					//foreach ($hh['acl'] as $h)
					$_category_acl    = $hh['acl'];
					//$_category_acl=!empty($h)?join(',', $h):'';

					$_category_parent = $hh['parent'];
					$_category_level   = $hh['level'];
					//$_category_level   = $categories[$category_id]['level'];
					
					//print "F_dashbroad 628 bien category_ids: $h => $jh <br>";
					//foreach ($hh['ac'] as $rt =>$jh)
					//$acl_ac=!empty($jh)?join(',', $jh):'';
					$acl_ac=!empty($hh['ac'])?join(',', $hh['ac']):'';
					
					//$acl_ac = join(',', $hh['ac']);
					//$keys_ac = explode(',', $hh['ac']);
					//$_category_acl    = spsep($categories[$category_id]['acl']);
					//foreach ($hh['acl'] as $h)
					//$keys_acl = explode(',', $hhh['acl']);
					//$acl_acl=!empty($keys_acl)?join(',', $keys_acl):'';
					//--------------------------------------
					print "<br>F_dashbroad 661 bien categories id-->--------------------$id----------------<br>";
					print "F_dashbroad 661 bien categories hh[id]-->--------------------".$hh['id']."----------------<br>";
					print "F_dashbroad 661 bien categories: name=>" . $_category_name ."<br>";
					print "F_dashbroad 661 bien categories: memo=>" . $_category_memo ."<br>";
					print "F_dashbroad 661 bien categories: icon=>" . $_category_icon."<br>";					
					print "F_dashbroad 661 bien categories: acl=>" .  $_category_acl."<br>";
					print "F_dashbroad 661 bien categories: parent=>" . $_category_parent."<br>";
					print "F_dashbroad 661 bien categories: level=>" .  $_category_level."<br>";
					print "F_dashbroad 661 bien categories: ac=>" .  $acl_ac."<br>";
									

					//$myupban ="UPDATE `admins` SET `ban`=0, numlogin = 1, lastdate = NOW() WHERE username = '$name';";
					$mesw ="UPDATE `forums` SET `name`='"
					//. $_category_id //id
					. $_category_name //$_POST['username']
					. "', `memo`='" . $_category_memo //$_POST['username']
					. "', `icon`='" . $_category_icon //$_POST['password1']
					. "', `acl`='" . $_category_acl //$_POST['email']
					. "', `parent`='" . $_category_parent //nick
					. "', `level`='" . $_category_level //e-hide
					. "', `ac`='" . $acl_ac //acl
	...
	if(!empty($categories_return)){
		foreach ($categories_return as $cm=> $hh){
			...
			$categoriesreturn[$_cm]['name'] = $hh['name'];
			$categoriesreturn[$_cm]['memo'] = $hh['memo'];
			$categoriesreturn[$_cm]['icon'] = $hh['icon'];
			$categoriesreturn[$_cm]['acl'] = $hh['acl'];
			$categoriesreturn[$_cm]['parent'] = $hh['parent'];
			$categoriesreturn[$_cm]['level'] = $hh['level'];
			$categoriesreturn[$_cm]['ac'] = $hh['ac'];
			$categoriesreturn[$_cm]['description'] = $hh['description'];
		}
	...
	//if ($category_id){
		...
		//if(is_array($category_id,$categoriesreturn,true)){
			$category_name   = $categoriesreturn[$category_id]['name'];
			$category_memo   = $categoriesreturn[$category_id]['memo'];
			$category_icon   = $categoriesreturn[$category_id]['icon'];
			$category_description   = $categoriesreturn[$category_id]['description'];
			$category_parent = $categoriesreturn[$category_id]['parent'];
			$category_acl    = spsep($categoriesreturn[$category_id]['acl']);
		}
	}
    // ---
...
if ($category_id){
	if(array_key_exists($category_id,$categories_call)){
        $category_name   = $categories_call[$category_id]['name'];
        $category_memo   = $categories_call[$category_id]['memo'];
        $category_icon   = $categories_call[$category_id]['icon'];
        $category_description   = $categories_call[$category_id]['description'];
        $category_parent = $categories_call[$category_id]['parent'];
        $category_acl    = spsep($categories_call[$category_id]['acl']);
    }
}
*/


	if(array_key_exists($category_id,$categoriesreturn)){echo "F_dashbroad 1004 -------------------- ok id---------------------- <br>";}
	else echo "F_dashbroad 1004 ----------------------no found id--------------------<br>";
	//----------------------------------------------
		print "F_dashbroad 741 kiem tra category_id: $category_id <br>";
		print "F_dashbroad 741 kiem tra category_name: $category_name <br>";
		print "F_dashbroad 741 kiem tra category_parent: $category_parent <br>";
		//print "F_dashbroad 741 kiem tra category_acl: $category_acl <br>";
		//----------------------------------------------
	...
	$category_id, $categoriesreturn, $category_name, $category_memo, $category_icon, $category_description, $category_acl, $category_parent, $groups);
