		...
		if($forresult ->num_rows!=0){
			while($forrow = $forresult ->fetch_assoc()){
				...
				if($nuresult ->num_rows!=0){
					...
					//}
						?>
						<option id="<?php echo $numenewtopicuser['user_id']; ?>" value="<?php echo $numenewtopicuser['user_id']; ?>"><?php echo $numenewtopicuser['username']?> (<?php echo $numenewtopicuser['mynum'] ?>) </option>
						<?php
					}//exit();
				}//exit();
			}
