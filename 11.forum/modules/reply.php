<?php

session_start();

require("../config.php");
require("../includes/functions.php");

//$db = mysql_connect($dbhost, $dbuser, $dbpassword);
//mysql_select_db($dbdatabase, $db);

if(isset($_GET['id']) == TRUE) {
	if(is_numeric($_GET['id']) == FALSE) {
		$error = 1;
	}

	if(isset($error) == 1) {
		header("Location: " . $config_basedir);
	}
	else {
		$validtopic = $_GET['id'];
	}
}
else {
	header("Location: " . $config_basedir);
}

if((isset($_SESSION['USERNAME']) == FALSE) && (isset($_SESSION['ADMIN']) == false)) {
	header("Location: " . $config_basedir . "/modules/login.php?ref=reply&id=" . $validtopic);
}

if(isset($_POST['submit'])) {
	$messagesql = "INSERT INTO messages(date, user_id, topic_id, subject, body) VALUES(NOW()
		, " . $_SESSION['USERID']
		. ", " . $validtopic
		. ", '" . $_POST['subject']
		. "', '" . $_POST['body']	
		. "');";

	mysqli_query($db,$messagesql);
	header("Location: " . $config_basedir . "/modules/viewmessages.php?id=" . $validtopic);
}
else {
	require("../header.php");
?>
	<form action="" method="post"><?php echo pf_script_with_get($SCRIPT_NAME); ?>
	<table>
	<tr>
		<td>Subject</td>
		<td><input type="text" name="subject"></td>
	</tr>
	<tr>
		<td>Body</td>
		<td><textarea name="body" rows="10" cols="50"></textarea></td>
	</tr>
	<tr>
		<td></td>
		<td><input type="submit" name="submit" value="Post!"></td>
	</tr>
	</table>
	</form>

<?php
}

require("../footer.php");

?>