<h2>Các loại tuyệt chiêu</h2>
- Bổ sung những tuyệt chiêu mới mạnh mẽ hơn các   tuyệt chiêu mà nhân vật đã sử dụng trước đây.<br>
<br>
- Có thể đạt được hòn   đá,pháp thư,thánh chỉ mới từ Monster có cấp độ trên 100Lv. <br>
<br>
<br>
<table cellspacing="0" cellpadding="10" align="center" border="1">
    <tbody>
    <tr>
        <td>Tên</td>
        <td>Sử dụng</td>
        <td>Hình ảnh</td>
        <td>Class</td>
        <td>Sản phẩm đạt được</td>
        <td>Yêu cầu</td>
    </tr>
    <tr>
        <td>Chém Băng</td>
        <td><img src="/images/guides/dk_StrikeofDestruction_02.jpg" ></td>
        <td><img src="/images/guides/img17C.gif"></td>
        <td>Chiến Binh</td>
        <td>Hòn đá Stroke</td>
        <td>÷</td>
    </tr>
    <tr>
        <td colspan="6">Đối tượng khi bị trúng đòn, trạng thái cơ thể chuyển sang lạnh và tốc độ di chuyển giảm xuống.
        </td>
    </tr>
    <tr>
        <td>Tăng ma lực</td>
        <td><img src="/images/guides/dw_Expansion-of-Wizardry_02.jpg" ></td>
        <td><img src="/images/guides/img17E.gif"></td>
        <td>Pháp Sư</td>
        <td>Pháp thư tăng ma lực</td>
        <td>Năng lượng: 1058</td>
    </tr>
    <tr>
        <td colspan="6">Tăng ma lực trong thời gian là sử dụng là 30 phút</td>
    </tr>
    <tr>
        <td>Liên Hoàn Tiễn</td>
        <td><img src="/images/guides/elf_Multi-Shot_02.jpg" ></td>
        <td><img src="/images/guides/img180.gif"></td>
        <td>Thánh Nữ</td>
        <td>Hòn đá multi shot</td>
        <td>Sử dụng vũ khí</td>
    </tr>
    <tr>
        <td colspan="6">Với 5 mũi tên bắn ra có thể xuyên qua đối thủ gây thiệt hại nhiều mục tiêu cùng một lúc.</td>
    </tr>
    <tr>
        <td>Hồi phục</td>
        <td><img src="/images/guides/elf_Recovery_02.jpg" ></td>
        <td><img src="/images/guides/img182.gif"></td>
        <td>Thánh Nữ</td>
        <td>Hòn đá hồi phục</td>
        <td>Năng lượng 340</td>
    </tr>
    <tr>
        <td colspan="6">Hồi phục SD của nhân vật.</td>
    </tr>
    <tr>
        <td>Quét Lửa</td>
        <td><img src="/images/guides/mg_Flame-Strike_02.jpg" ></td>
        <td><img src="/images/guides/img184.gif"></td>
        <td>Đấu Sĩ</td>
        <td>Hòn đá Flame Strike</td>
        <td>Trên 220Lv</td>
    </tr>
    <tr>
        <td colspan="6">Thanh gươm lửa có thể gây sát thươngtrong phạm vi xung quanh.</td>
    </tr>
    <tr>
        <td>Bão Điện</td>
        <td><img src="/images/guides/mg_Gigantic-Storm_02.jpg" ></td>
        <td><img src="/images/guides/img186.gif"></td>
        <td>Đấu Sĩ</td>
        <td>Pháp thư Gigantis</td>
        <td>Năng lượng 1058</td>
    </tr>
    <tr>
        <td colspan="6">Tạo tia chớp có khả năng gây sát thương xung quanh.</td>
    </tr>
    <tr>
        <td>Chaotic Decayer</td>
        <td><img  src="/images/guides/dl_ChaoticDiseier_02.jpg" ></td>
        <td><img src="/images/guides/img188.gif"></td>
        <td>Chúa Tể</td>
        <td>Thánh chỉ Chaotic Decayer</td>
        <td>Trên 220Lv <br>
            Năng lượng: 160/td&gt; </td>
    </tr>
    <tr>
        <td colspan="6">Bạn có thể gây thiệt hại lên đối thủ ngay cả trong bóng đêm.</td>
    </tr>
    </tbody>
</table>