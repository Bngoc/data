...
function cn_helper_html_text($e, $p)
{
    $text = $e[$p];
    //-------------------------------------------
    print " F_news bien text: " . $text . "<br>";
    print " F_news bien p: " . $p . "<br>";
    foreach ($e as $fs => $gd) {
        print " F_news bien e: $fs => " . $gd . "<br>";
    }
    print " F_news bien texts: " . $text . "<br>";
    //----------------------------------------------
    // replace word module
    ...
    if (isset($e['ht']) && $e['ht']) {
        ...
        $text = nl2br(cnHtmlSpecialChars($text));
    }
    ...
    do {
        if (preg_match_all('/\[([a-z0-9_-]+)(.*?)\](.*?)\[\/\\1\]/is', $text, $cp, PREG_SET_ORDER)) {
            foreach ($cp as $ci) {
                ...
                print "00000000000000000000000" . $func . "<br>";
    ...
    print " -----------------------------------------<br>";
    print "F_news 86 bien text: " . $text . "<br>";
...
function cn_helper_bb_decode($bb)
{
    ...
    if (isset($bb['target']) && $bb['target']) {
        $a_opts = 'target="' . cnHtmlSpecialChars($bb['target']) . '" ';
    }
    if (isset($bb['anchor']) && $bb['anchor']) {
        $b_opts = '#' . cnHtmlSpecialChars($bb['anchor']);
    }
...
function cn_modify_s2bb_b($t)
{
    return '<strong>' . $t . '</strong>';
}

function cn_modify_s2bb_i($t)
{
    return '<em>' . $t . '</em>';
}

function cn_modify_s2bb_u($t)
{
    return '<span style="text-decoration: underline;">' . $t . '</span>';
}

function cn_modify_s2bb_color($t, $o)
{
    return '<span style="color: ' . preg_replace('/[^a-z0-9\-\_\#]/i', '', substr($o, 1)) . ';">' . $t . '</span>';
}

function cn_modify_s2bb_size($t, $o)
{
    return '<span style="font-size: ' . intval(substr($o, 1)) . 'pt;">' . $t . '</span>';
}

function cn_modify_s2bb_font($t, $o)
{
    return '<span style="font-family: ' . preg_replace('/[^a-z0-9\-\_]/i', '', substr($o, 1)) . ';">' . $t . '</span>';
}

function cn_modify_s2bb_align($t, $o)
{
    return '<div style="text-align: ' . preg_replace('/[^a-z\-]/i', '', substr($o, 1)) . ';">' . $t . '</div>';
}
...
function cn_modify_s2bb_more($t)
{
    ...
    $echo = '<div class="cn_more_button"><a href="#" onclick="cn_more_expand(\'more_link_' . $ID . '\'); return false;">' . __("expand_more") . '</a></div>';
...
function cn_modify_author($e)
{
    ...
    if (is_null($user)) {
        return cnHtmlSpecialChars($e['u']);
    }

    // base username
    $username = cnHtmlSpecialChars($e['u']);

    // user has nick
    if (isset($user['nick']) && $user['nick']) {
        $username = cnHtmlSpecialChars($user['nick']);
    }
...
function cn_modify_category($e)
{
    ...
    return cnHtmlSpecialChars(join(', ', $ns));
}
...
function cn_modify_tagline($e)
{
    ...
    foreach ($x as $tag) {
        ...
        $echo[] = str_replace('{tag}', cnHtmlSpecialChars($tag), $esrc);
...
function cn_modify_page_alias($e)
{
    return $e['pg'];
}

function cn_modify_phpself()
{
    global $PHP_SELF;
    return $PHP_SELF;
}

function cn_modify_go_back()
{
...
function cn_modify_category_ids($e)
{
    return $e['c'];
}

function cn_modify_category_id($e)
{
    return intval($e['c']);
}
...
// --- frontend accessible ---
function cn_modify_bb_img($e, $t, $bb)
{
    return cn_modify_s2bb_img($t, $bb);
}

function cn_modify_bb_more($e, $t)
{
    return cn_modify_s2bb_more($t);
}

function cn_modify_bb_youtube($e, $t)
{
    return cn_modify_s2bb_youtube($t);
}
...
function cn_modify_comm_author($e)
{
    ...
    // User exists
    $username = $user['name'] ? cnHtmlSpecialChars($username) : 'Anonymous';
...
function cn_modify_comm_input_username()
{
    ...
    // User is authorized
    if ($user) {
        return '<input type="text" class="cn_comm_username" value="' . cnHtmlSpecialChars($user['name']) . '" disabled="disabled" />';
    } else {
        $guest_name = isset($_SESSION['guest_name']) ? $_SESSION['guest_name'] : '';
        $name_input = REQ('name', 'POST') ? REQ('name', 'POST') : $guest_name;
        return '<input type="text" class="cn_comm_username" name="name" value="' . cnHtmlSpecialChars($name_input) . '"/>';
    }
}
...
function cn_modify_comm_input_email()
{
    ...
    // User is authorized
    if ($user) {
        return '<input type="text" class="cn_comm_email" value="' . cnHtmlSpecialChars($user['email']) . '" disabled="disabled" />';
    } else {
        $guest_name = isset($_SESSION['guest_name']) ? $_SESSION['guest_name'] : '';
        $guest_email = isset($_SESSION['guest_email']) ? $_SESSION['guest_email'] : '';

        $email_input = REQ('mail', 'POST') ? REQ('mail', 'POST') : $guest_email;
        return '<input type="text" name="mail" class="cn_comm_email" value="' . cnHtmlSpecialChars($email_input) . '"/>';
    }
}
...
function cn_modify_comm_input_commentbox($e)
{
    ...
    return '<textarea cols="40" rows="6" name="comments" class="cn_comm_textarea" id="ncomm_' . $e['id'] . '">' . cnHtmlSpecialChars($cm_text) . '</textarea>';
}
...
function cn_modify_comm_remember_me()
{
    ...
    $echo = '<input class="cn_comm_remember" type="checkbox" name="cn_remember_me" value="Y" /> ' . __('remember_me') . ' ';
    if ($member_name) {
        $echo .= '<input type="hidden" name="isforgetme" value="" />';
        $echo .= '<span class="cn_comm_forget"><a href="#" onclick="forget_me(); return false;">' . __('forget_me') . '</a></span>';
    }

    if ($name) {
        $echo = '<span class="cn_com_logged">' . __('logged_as') . ' <b>' . cnHtmlSpecialChars($name) . '</b></span>';
        $echo .= ' <input type="hidden" name="isforgetme" value="" />';
        $echo .= ' <span class="cn_comm_forget"><a href="#" onclick="forget_me(); return false;">' . __('forget_me') . '</a></span>';
    }
...
{
    $echo = '<input type="submit" value="' . cnHtmlSpecialChars($t) . '" class="cn_submit_bb"/>';
