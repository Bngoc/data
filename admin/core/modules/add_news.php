...
//                db_update_aux_admin($entry, 'add');
                ...
                if (getOption('notify_status') && getOption('notify_unapproved') && testRoleAdmin('Bs')) {
                    cn_send_mail(
                        ...
                        cn_replace_text(cn_get_template('notify_unapproved', 'mail'), '%username%, %article_title%', $member['user_Account'], $title)
                    );
                }

                // view in editor
                cnRelocation(PHP_SELF . '?mod=editnews&action=editnews&id=' . $c_time . '&m=added');
            }
        } else {
            // show news preview
    ...
    // ---
    echo_header_admin("com_news@skins/mu_style.css", __("addnews"));
