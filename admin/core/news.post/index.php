 ...
 * UTF-8 CN: I18N, Security & bug fixes - korn19.ch (2009)
 *********************************************/
...
if ($config_use_cookies == TRUE) {
    ...
    if (isset($username)) {
        if (isset($HTTP_COOKIE_VARS['md5_password'])) {
            $cmd5_password = $HTTP_COOKIE_VARS['md5_password'];
        } elseif (isset($_COOKIE['md5_password'])) {
            $cmd5_password = $_COOKIE['md5_password'];
        } else {
            $cmd5_password = md5($utf8_salt . md5($password) . $_SERVER['REMOTE_ADDR']);
        }
...
if ($config_use_sessions == TRUE) {
    ...
    if ($action == 'dologin') {
        $md5_password = md5($utf8_salt . md5($password) . $_SERVER['REMOTE_ADDR']);
...
if ($is_loged_in == FALSE) {
    ...
    if ($config_allow_registration == 'yes') {
        $allow_reg_status = '<a href="register.php">(' . $say['register'] . ')</a> ';
    } else {
        $allow_reg_status = '';
    }
