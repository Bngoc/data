<?php
if ($member_db[1] > 3 or ($member_db[1] != 1 and $action == 'doimagedelete')) {
...
<script language=\"javascript\" type=\"text/javascript\">
	...
	";
            ...
            echo "opener.document.getElementById(area).value += finalImage;";
        }
...
// *************
    if ($subaction == 'upload') {
        for ($image_i = 1; $image_i < ($images_number + 1); $image_i++) {
            ...
            if ($image_name == '') {
                $img_result .= "<br><font color=red>$current_image -> No File Specified For Upload!</font>";
            } elseif (!isset($overwrite) and file_exists($config_path_image_upload . "/" . $image_name)) {
                $img_result .= "<br><font color=red>$image_name -> Image already exist!</font>";
...
<table width=100% height=100% cellspacing=0 cellpadding=0>";
    ...
    while ($file = readdir($img_dir)) {
        //Store them in array for sorting
        $images_in_dir[] = $file;
    }
