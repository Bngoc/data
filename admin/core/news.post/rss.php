...
if(!isset($rss_news_include_url) or !$rss_news_include_url or $rss_news_include_url == ''){
	die("The RSS is not configured.<br>Please do this in: <b>CuteNews &gt; Options &gt; Implementation Wizards &gt; RSS</b>");
}
...
if(!$_GET['number'] or !preg_match('/^[1-9]{1}[0-9]{0,}$/', $_GET['number'])){ $number = 10; }
else{ $number = trim($_GET['number']); }
if(!$_GET['only_active'] or $_GET['only_active'] == ''){ $only_active = TRUE;}
else{ $only_active = $_GET['only_active'];}
