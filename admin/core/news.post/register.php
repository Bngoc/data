...
if($action == 'doregister'){
	...
	if(!preg_match("/^[\.A-z0-9_-]{1,15}$/i", $regpassword)){ msg('error',$say['error'], $say['invalid_pass']); }
...
elseif($action == 'validate'){
	...
	if(!$found){ msg('error', $say['error'], $say['unmatch']); }
	else{
...
echo<<<HTML
    <table leftmargin=0 marginheight=0 marginwidth=0 topmargin=0 border=0 height=100% cellspacing=0>
     ...
     <tr>
       ...
       <td><input tabindex=1 type="password" name=regpassword style="width:134" size="20"></td>
     </tr>
     <tr>
