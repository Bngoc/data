...
ef") . "</p>";//'
  ...
  KEY `myindex` (`row1`,`row2`)
    ...
    if ($rs && !$rs->EOF) {
        ...
        if (!$rs->EOF) {
            Err("Error EOF");
        } elseif (is_array($rs->fields) || $rs->fields) {
    ...
    if ($ret != 1) {
        if ($db->dataProvider == 'ibase' && $ret == 2) ; else {
            print "<b>Replace failed: </b>";
        }
    }
...
This script tests the following databases: Interbase, Oracle, Visual FoxPro, Microsoft Access (ODBC and ADO), MySQL,
MSSQL (ODBC, native, ADO).
...
<p>
    ...
    ?>
<p><i>ADODB Database Library (c) 2000-2014 John Lim. All rights reserved. Released under BSD and LGPL,
        PHP <?php echo PHP_VERSION ?>.</i></p>
</body>
</html>
