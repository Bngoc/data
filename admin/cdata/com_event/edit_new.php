...
//list($editconfigs, $editcats, $editforums, $editnews, $editusers, $entries_total, $userlist) = _GL('editconfigs, editcats, editforums, editnews, editusers, entries_total:intval, userlist'); // tabs menu
    list($mod, $act, $page, $per_page, $source, $entries_total, $showed, $ndraft) = _GL('mod, act, page, per_page, source, entries_total, num_showview, numdraft'); // cho duong dan 
    ...
    list($arrayyear, $arraymon, $arrayday, $arraynumrecode, $datalist, $paginationview, $_cat, $_forum) = _GL('arrayyear, arraymon, arrayday, arraynumrecode, datalist, pagination, cat_label, forum_label');
    ...
    */
    ...
    <div class="panel">
        <div style="float: right;">
            Entries on page: <?php foreach (array(25, 50, 100, 250) as $_per_page) {
                echo ' <a href="' . cn_url_modify('mod=editnews', 'page', "per_page=$_per_page") . '" ' . ($per_page == $_per_page ? 'class="b"' : '') . '>' . $_per_page . '</a> ';
            } ?>
    ...
    <div class="source">
        ...
        Source:
        <a href="<?php echo cn_url_modify('mod=editnews', 'source,page'); ?>" <?php if ($source == '') {
            echo 'class="b"';
        } ?>>Active</a> /
        <a href="<?php echo cn_url_modify('mod=editnews', 'page', 'source=draft'); ?>" <?php if ($source == 'draft') {
            echo 'class="b"';
        } ?> > Draft (<?php echo $ndraft; ?>) </a>
        <div style="clear:both;"></div>

        <div style="<?php if (empty($forums_filters) && empty($users_filters)) {
            echo 'display: none;';
        } ?>" id="filters">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
                ...
                <table width="100%">
                    <tr>
                        <td>By sub forum of catagoris</td>
                        <td>By user</td>
                    </tr>
                    <tr>
                        <td style="width: 45%;">
                            <select name="forum_filters" style="width: 100%;" class="subforum">
                                ...
                                if (count($_cat) > 0) {
                                    asort($_cat);
                                    foreach ($_cat as $rt => $tr) {

                                        //$catsql = "SELECT * FROM categories;";
                                        //$catresult =$db->query($catsql);
                                        //if($catresult ->num_rows!=0){
                                        //while($catrow = $catresult ->fetch_assoc()){
                                        $getidcat = intval($tr['id']);
                                        $itemname = UTF8ToEntities($tr['name']);
                                        ?>
                                        <optgroup label="<?php echo $itemname; ?>" style="font:bold;">
                                            ...
                                            if (count($_forum) > 0) {
                                                ...
                                                foreach ($_forum as $jk => $kj) {
                                                    ...
                                                    if ($kj['cat_id'] == $getidcat) {
                                                        $getidfor = intval($kj['id']);
                                                        $namefor = UTF8ToEntities($kj['name']);

                                                        ?>
                                                        <option id="<?php echo $getidfor; ?>"
                                                                value="<?php echo $getidfor; ?>"><?php echo $namefor; ?></option>
                                                        <?php
                                                    }
                                                }
                                            } ?>
                    ...
                    <tr>
                        ...
                        <td><?php
                            if (count($forums_filters) > 0) {
                                foreach ($forums_filters as $id) {
                                    if (isset($_forum[$id])) {
                                        echo ' [<a href="' . cn_url_modify('mod=editnews', "rm_forum_filter=$id") . '" style="color: red;">&ndash;</a>] <b>' . UTF8ToEntities($_forum[$id - 1]['name']) . '</b> &nbsp; ';
                                    }
                                }
                            }
                            ?>
                        ...
                        <td><?php
                            if (count($users_filters) > 0) {
                                //arsort($users_filters);
                                foreach ($users_filters as $id => $val) {
                                    ...
                                    $id_val = $val['id'];
                                    echo ' [<a href="' . cn_url_modify('mod=editnews', "rm_user_filter=$id_val") . '" style="color: red;">&ndash;</a>] <b>' . UTF8ToEntities($val['username']) . '</b> &nbsp; ';
                                    //}
                                }
                            }
                            ?>
            ...
            <tr valign="top">
                ...
                <td style="background: #f8f8f8; border: 0; display: block;width:15%" id="browse_dates">
                    ...
                    <table class="std-table" width="100%">
                        <tr>
                            <th>
                                <nobr><a href="<?php echo cn_url_modify('mod=editnews', 'mon,day,year'); ?>">All
                                        periods</a></nobr>
                            </th>
                        </tr>
                        <tr>
                            <td>
                                ...
                                if (count($arrayyear) > 0) {
                                    foreach ($arrayyear as $qw => $wq) {
                                        ...
                                        <?php if ($YS == $yc) {
                                            if (count($arraymon) > 0) {
                                                foreach ($arraymon as $fq => $qf) {
                                                    //$resultm =$db->query($myy_mon);
                                                    //if($resultm ->num_rows !=0){
                                                    //while($rowm = $resultm ->fetch_assoc()){
                                                    $mc = $qf['mon']; ?>
                                                    <div class="mons<?php if ($mc == $MS) {
                                                        echo '-b' . ($DS ? '' : 'wm');
                                                    } ?>">
                                                    ...
                                                    <?php if ($mc == $MS) {
                                                        if (count($arrayday) > 0) {
                                                            foreach ($arrayday as $fr => $rf) {
                                                                ...
                                                                ?>
                                                                <div class="days<?php if ($dc == $DS) {
                                                                    echo '-b';
                                                                } ?>">
                                                                    <a href="<?php echo cn_url_modify('mod=editnews', "year=$yc", "mon=$mc", "day=$dc"); ?>"><?php echo $dc; ?></a>
                                                                    <?php if (count($arraynumrecode) > 0) {
                                                                        echo '<small>(' . $arraynumrecode[$fr]['numrecode'] . ')</small>';
                                                                    } ?>
                                                                </div>
                                                            <?php }
                                                        }
                    ...
                    <table class="std-table" width="100%">
                        <tr>
                            <th>
                                <nobr><a href="<?php echo cn_url_modify('mod=editnews', 'forum_filters'); ?>">All
                                        forums</a></nobr>
                            </th>
                        </tr>
                        <tr>
                            <td>
                                <div>
                                    <a <?php //if ($cat_filter === '-') { echo 'class="bold" '; } 
                                       ?>href="<?php echo cn_url_modify('mod=editnews', 'forum_filters'); ?>">Free news
                                        only</a>
                                </div>
                                <hr/>
                                <?php
                                if (count($_forum) > 0) {
                                    //asort($_forum);
                                    foreach ($_forum as $jko => $kjo) {
                                        $id = $kjo['id'];
                                        // foreach ($category as $id => $cat)
                                        echo '<div style="word-wrap:break-word; width: 200px;"><a ' . (in_array($id, $forums_filters) ? 'class="bold" ' : '') . 'href="' . cn_url_modify('mod=editnews', "forum_filters=$id") . '">' . UTF8ToEntities($kjo['name']) . '</b></div>';
                    ...
                    <table width="100%" class="std-table">
                        ...
                        if (count($datalist) > 0) {
                            ...
                            <tr>
                                ...
                                ?>
                                <th width="1" align="center"><input style="border: 0; background: transparent;"
                                                                    type=checkbox name=master_box title="Check All"
                                                                    onclick="check_uncheck_all('selected_news[]');">
                                </th>
                            </tr>

                            <?php foreach ($datalist as $id => $rowedit) {
                                ...
                                <tr style="background: <?php echo($i++ % 2 ? '#F7F6F4' : '#FFFFFF'); ?>" class="hover">
                                    ...
                                    <td>
                                        <div style="word-wrap: break-word; */width:200px;*/">
                                            ...
                                            if ($rowedit['can']) {
                                                ...
                                                ?>
                                                <a title="<?php echo $subjects; ?>"
                                                   href="<?php echo cn_url_modify(array('reset'), 'mod=editnews', 'act=edit', "id=$topicid"); ?>"><?php echo $subjects; ?></a>
                                            <?php } else {
                                                echo $subjects;
                                            }
                                            ?>
                                    ...
                                    <td align='center'><?php $com = isset($rowedit['com']) ? $rowedit['com'] : $rowedit['com'];
                                        echo $rowedit['com']; ?></td>
                                    <td align=''><?php
                                        ...
                                        ?></td>
                                    <td align="center" title="<?php echo $rowedit['date']; ?>">
                                        <nobr><?php echo date("Y-m-d h:ia", strtotime($rowedit['date'])); ?></nobr>
                                    </td>
                                    <td align="center"><a
                                            href="<?php echo cn_url_modify('mod=editnews', 'user_filters=' . $rowedit['user_id']); ?>"><?php echo UTF8ToEntities($rowedit['username']); ?></a><sup>
                                    </td>
                                    <?php //hook('template/editnews/list_item_after', array($ID, $entry)); ?>
                                    <td align="center"><?php if ($rowedit['can']) { ?><input name="selected_news[]"
                                                                                             value="<?php echo $rowedit['topicid']; ?>"
                                                                                             style="border:0;"
                                                                                             type='checkbox'><?php } ?>
                                    </td>
        ...
        <div class="list-footer">
            <div class="actions">
                ...
                <select name="subaction"> <!--subaction-->
                    ...
                    <?php if (test('Nud')) { ?>
                        <option value="mass_delete">Delete news</option><?php } ?>
                    <?php if (test('Nua')) { ?>
                        <option value="switch_to_html">Switch to HTML</option><?php } ?>
