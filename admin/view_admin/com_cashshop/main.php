    ...
    <?php if ($preview_html_full) { ?>
        <div style="margin: 10px 0 30px 0;">
            <div style="border: 1px dashed gray; float: left; width: 50%;">
                <div
                    style="background: #eeeeee; border-bottom: 1px solid #cccccc; margin: 0; padding: 4px; text-align: center;">
                    <b>PREVIEW FULL STORY ENTRY</b></div>
                <div style="border:4px solid #f0f0a0;"><?php echo $preview_html_full; ?></div>
            </div>
            <div style="clear:left;"></div>
            <div style="height: 8px"></div>
        </div>
...
<?php } ?>
    ...
    <form method="post" action="<?php echo PHP_SELF; ?>">
        ...
        <?php if ($categories) { // Categories not exists?>
            <div class="section">
                <div class="name">Category [<a href="#" onclick="return(tiny_msg(this));"
                        ...
                        <?php foreach ($categories as $catid => $cat_data) {
                            if (empty($cat_data['ac_forum'])) { ?>
                             ...
                             <input id="chkbx<?php echo $catid ?>" type="checkbox"
                                    name="category[]" <?php if ($vCategory && in_array($catid, $vCategory)) {
                                 echo 'checked';
                             } ?>
        ...
        <div class="section">
            ...
            <?php if ($CKEDITOR_Active == 0) { ?>
                <div class="ballon">
                    ...
                    include SKIN . '/dashboard/snippet/editor.php'; ?>
        ...
        <!-- Full story -->
        <div class="section" id="full-story" <?php if (!$vFull) {
        ...
        <div class="section">
            ...
            <div style="float: right">
                ...
                <?php if (empty($_POST['full_story'])) { ?><input type=button onClick="ShowOrHide('full-story','');"
                                                                  value="Toggle Full-Story"><?php } ?>
                <input type=submit name="do_editsave" style='font-weight: bold' title="Post the New Article"
        ...
        <div class="section">
            ...
            <div class="name">Article meta [<a href="#" onclick="return(tiny_msg(this));"
                                               title="Some additional optional data">?</a>]
            </div>
            <hr/>
            ...
            <div>
                ...
                <div>
                    <?php if ($CKEDITOR_Active == 0 && $is_active_html) { ?>
                        <div><label for='html'> <input id='html' class="chkbox-fix" type="checkbox" value="1"
                                                       name="if_use_html" <?php if ($vUseHtml) {
                                    echo 'checked';
                                } ?>> Use
                                HTML in this article</label></div>
                    <?php } else { ?>
                        <?php if ($is_active_html) { ?>
                            <div><label for='html'> <input id='html' class="chkbox-fix" type="checkbox" value="1"
                                                           name="if_use_html" checked="checked" disabled="disabled"> Use
                                    HTML in this article</label></div>
                        <?php } else { ?>
                            <input type="hidden" name="if_use_html" value="1"/>
                        <?php } ?>
                    <?php } ?>
                    <div><label for='html'> <input id='concat' class="chkbox-fix" type="checkbox" value="Y"
                                                   name="concat" <?php if ($vConcat) {
                                echo 'checked';
                            } ?>> Concate short
                            and full story</label></div>
                </div>
                <br/>
                ...
                <div>
                    <?php if (!$archive_id) { ?>
                        ...
                        <?php if (!test('Bd')) { ?>
                            ...
                            <label for='active'><input checked id='active'
                                                       style="border:0; background-color: transparent" type=radio
                        ...
                        <label for='draft'><input id='draft' style="border:0; background-color: transparent" type=radio
                                                  value="draft" <?php if ($is_draft) {
                                echo "checked";
                            } ?>
