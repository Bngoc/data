...
<form action="<?php echo PHP_SELF; ?>" method="POST" accept-charset="utf-8">
    ...
    <table class="std-table wide">
        ...
        <?php if ($categories) {
            foreach ($categories as $id => $category) {
                ...
                $id_vl = $category['id']; ?>
                <tr <?php if ($id_vl == $category_id) {
                    echo ' class="row_selected"';
                } ?>>
                    <td align="center"><?php echo intval($id_vl); ?></td>
                    <td><?php echo str_repeat('&ndash;&nbsp;', $category['level_forum']); ?> <a
                    ...
                    <td align="center"><?php if ($category['icon_forum']) {
                            echo '<img style="max-width: 128px; max-height: 128px;" src="' . ($category['icon_forum']) . '" />';
                        } else {
                            echo '---';
                        } //cn_htmlspecialchars ?></td>
    ...
    <table class="panel wide">
        <tr>
            ...
            <td><input style="width: 55%;" type="text" name="category_name" value="<?php echo($category_name); ?>"/>
            </td>
        </tr>
                    ...
                    <?php if ($categories) {
                        foreach ($categories as $id => $category) {
                            if ($category_id != $category['id']) { ?>
                                <option <?php if ($category_parent == $category['id']) {
                                    echo 'selected';
        ...
        <tr>
            ...
            <td>
                <textarea style="width: 100%;" rows="6" cols="24" id="category_description" name="category_description"
                          tabindex=""><?php echo($category_description); ?></textarea>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <?php foreach ($groups as $id => $name) { ?>
                    <input type="checkbox"
                           name="category_acl[]" <?php if ($category_acl && in_array($id, $category_acl)) {
                        echo 'checked';
                    } ?>
