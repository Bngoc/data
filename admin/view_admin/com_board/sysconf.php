    ...
    <?php foreach ($options_list as $ID => $ol) { ?>
        <li <?php if ($sub == $ID) {
            echo ' class="selected"';
                ...
                href="<?php echo cn_url_modify('mod=editconfig', 'opt=sysconf', "sub=$ID"); ?>"><?php echo ucfirst($ID); ?></a>
        </li>
    <?php } ?>
</ul>
    ...
    <table class="std-table opt_table">
        ...
        <?php foreach ($options as $opt_id => $opt_vars) {
            ...
            <tr <?php if ($odd++ % 2) {
                ...
                <td>
                    <div class="o_title">
                        <b><?php echo $opt_vars['title']; ?></b> <?php if ($opt_vars['help']) {
                ...
                <td align="center"><?php
                    ...
                    if ($opt_vars[0] == 'label') {
                        echo cnHtmlSpecialChars($opt_vars['var']);
                    } elseif ($opt_vars[0] == 'disable') {
                        echo '<input type="text" disabled style="width: 500px;" value="' . cnHtmlSpecialChars($opt_vars['var']) . '"/>';
                    } elseif ($opt_vars[0] == 'text') {
                        echo '<input type="text" name="config[' . $opt_id . ']" style="width: 400px;" value="' . cnHtmlSpecialChars($opt_vars['var']) . '"/>';
                    } elseif ($opt_vars[0] == 'int') {
                        echo '<input type="text" name="config[' . $opt_id . ']" size="28" value="' . intval($opt_vars['var']) . '"/>';
                    } elseif ($opt_vars[0] == 'password') {
                        echo '<input type="password" name="config[' . $opt_id . ']" size="28" value=""/>';
