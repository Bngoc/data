...
<form action="<?php echo PHP_SELF; ?>" method="POST">
    ...
    <table class="panel" width="100%">
        ...
        <tr>
            <td align="right">Password <?php if (!$user_name || ($user_name && !$is_edit)) { ?><span
                    class="required">*</span><?php } ?></td>
        ...
        <tr>
            <td align="right">Confirm <?php if (!$user_name || ($user_name && !$is_edit)) { ?><span
                    class="required">*</span><?php } ?></td>
            ...
            <td>
                <select name="user_acl">
                    <?php foreach ($grp as $grp_id => $item) { ?>
                        <option
                            value="<?php echo $grp_id; ?>"<?php if ($user_acl == $grp_id) {
                            echo ' selected="selected"';
                        } ?>><?php echo cnHtmlSpecialChars(ucfirst($item['N'])); ?></option>
                    <?php } ?>
                </select>
                <button name="add" value="add">Add</button>
                <?php if ($user_name && $is_edit) { ?>
                    <button name="edit" value="edit">Edit</button><?php } ?>
            </td>
...
<table class="std-table" width="100%">
    ...
    <?php if ($users) {
        foreach ($users as $id => $user) { ?>
            <tr <?php if (isset($user['name']) && $user_name == $user['name']) {
...
<div style="text-align: right; margin: 16px 0 0 0;">
    <a class="external" onclick="<?php echo cn_snippet_open_win(PHP_SELF . '?mod=help&section=users'); ?>" href="#">Understanding user levels</a>
</div>
