			...
			<table>
				...
				<tr>
					<td><select name="add_category_filter">
                            ...
                            <?php foreach ($category as $catid => $cat) {
                                echo '<option value="' . $catid . '">' . cnHtmlSpecialChars($cat['name']) . '</option>';
                            } ?>
						</select>
					</td>
					...
					<td><select name="add_user_filter">
							<option value="">--</option>
                            <?php foreach ($userlist as $user => $num) {
                                echo '<option value="' . cnHtmlSpecialChars($user) . '">' . cnHtmlSpecialChars($user) . ' (' . $num . ')</option>';
                            } ?>
						</select>
					</td>
				...
				<tr>
					...
					<td>
                        <?php foreach ($category_filters as $id) {
                            echo ' [<a href="' . cn_url_modify('cat_filter', "rm_category_filter=$id") . '" style="color: red;">&ndash;</a>] <b>' . $category[$id]['name'] . '</b> &nbsp; ';
                        }
                        ?>
					</td>

					<td>
                        <?php foreach ($user_filters as $id) {
                            echo ' [<a href="' . cn_url_modify("rm_user_filter=$id") . '" style="color: red;">&ndash;</a>] <b>' . cnHtmlSpecialChars($id) . '</b> &nbsp; ';
                        }
                        ?>
					</td>
				</tr>
		...
		<tr valign="top">
			...
			<td style="background: #f8f8f8; border: 0; display: block;" id="browse_dates">
				...
				<table class="std-table" width="100%">
					<tr>
						<th>
							<nobr><a href="<?php echo cn_url_modify('mon,day,year,archive_id'); ?>">All periods</a>
							</nobr>
						</th>
					</tr>
					<tr>
						<td>
                            <?php if ($source == 'archive') { ?>
								...
								<div id="archives_list">
                                    <?php foreach ($ptree as $id => $item) { ?>
										<a class="arch<?php if ($archive_id == $id) echo '-b'; ?>"
										   href="<?php echo cn_url_modify('mon,day', "archive_id=$id"); ?>"><em><?php echo date('Y-m-d H:i', $id); ?></em> <?php if ($item['c']) echo '<small>(' . intval($item['c']) . ')</small>'; ?>
										</a>
                                    <?php } ?>
								</div>
                            ...
                            <?php } else {
                                foreach ($TY as $yc => $cy) { ?>
									<div class="years<?php if ($yc == $YS) echo '-b' . ($MS ? '' : 'wm'); ?>"><a
												href="<?php echo cn_url_modify('mon,day', "year=$yc"); ?>"><?php echo $yc; ?></a>
									</div>
                                    <?php if ($YS == $yc) { ?>
                                        <?php foreach ($TM as $mc => $cm) { ?>
											<div class="mons<?php if ($mc == $MS) echo '-b' . ($DS ? '' : 'wm'); ?>"><a
														href="<?php echo cn_url_modify('day', "year=$yc", "mon=$mc"); ?>"><?php echo cn_modify_month($mc); ?></a>
											</div>
                                            <?php if ($mc == $MS) { ?>
                                                <?php foreach ($TD as $dc => $cd) { ?>
													<div class="days<?php if ($dc == $DS) echo '-b'; ?>"><a
																href="<?php echo cn_url_modify("year=$yc", "mon=$mc", "day=$dc"); ?>"><?php echo $dc; ?></a> <?php if ($cd) echo '<small>(' . $cd . ')</small>'; ?>
													</div>
                                                <?php }
                                            }
                                        }
                ...
                <?php if ($category) { ?>
					...
					<table class="std-table" width="100%">
						<tr>
							<th>
								<nobr><a href="<?php echo cn_url_modify('cat_filter'); ?>">All categories</a></nobr>
							</th>
						</tr>
						<tr>
							<td>
								<div>
									<a <?php if ($cat_filter === '-') echo 'class="bold" '; ?>href="<?php echo cn_url_modify('cat_filter=-'); ?>">Free
										news only</a>
								</div>
								<hr/>
                                <?php
                                foreach ($category as $id => $cat)
                                    echo '<div><a ' . ($id == $cat_filter ? 'class="bold" ' : '') . 'href="' . cn_url_modify("cat_filter=$id") . '">' . cnHtmlSpecialChars($cat['memo'] ? $cat['memo'] : $cat['name']) . '</b></div>';
                                ?>
							</td>
						</tr>
					</table>
                    ...
                    <?php if ($showed) { ?>
						...
						<tr>
							...
							<th width="1" align="center">
								<input style="border: 0; background: transparent;"
                        ...
                        <?php foreach ($entries as $ID => $entry) { ?>
							...
							<tr style="background: <?php echo $entry['is_pros'] ? '#E0E0E0' : ($i++ % 2 ? '#F7F6F4' : '#FFFFFF'); ?>"
							    class="hover">
								...
								<td>
                                    <?php if ($entry['can']) {
                                        ...
                                        ?>
										<a title="<?php echo cnHtmlSpecialChars($title . ($entry['pg'] ? ' (' . $entry['pg'] . ')' : '')); ?>"
										   href="<?php echo cn_url_modify('action=editnews', "id=$ID"); ?>"><?php echo cnHtmlSpecialChars($title); ?></a>
                                    <?php } else echo cnHtmlSpecialChars($entry['title']); ?>
								</td>
								...
								<td align='center'>
                                    ...
                                    ?></td>
								<td align="center" title="<?php echo $entry['date_full']; ?>">
									<nobr><?php echo $entry['date']; ?></nobr>
								</td>
								<td align="center">
									<a href="<?php echo cn_url_modify('add_user_filter=' . $entry['user']); ?>"><?php echo cnHtmlSpecialChars($entry['user']); ?></a><sup>
								</td>
                                <?php hook('template/editnews/list_item_after', array($ID, $entry)); ?>
								<td align="center">
                                    <?php if ($entry['can']) { ?>
										<input name="selected_news[]" value="<?php echo $ID; ?>" style="border:0;"
										       type='checkbox'>
                                    <?php } ?></td>
