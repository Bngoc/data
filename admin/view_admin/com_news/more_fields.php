    ...
    <?php foreach ($morefields as $section => $items) { ?>
        ...
        <?php foreach ($items as $name => $item) { ?>
            ...
            <?php if ($item['type'] == 'text') { ?>
                ...
                <div><input type="text" style="width: 100%" value="<?php echo cnHtmlSpecialChars($item['#value']); ?>" name="faddm[<?php echo $name; ?>]" ></div>

            <?php } elseif ($item['type'] == 'image') { ?>

                <div>
                    <input id="faddm_<?php echo $name; ?>" type="text" style="width: 500px" value="<?php echo cnHtmlSpecialChars($item['#value']); ?>" name="faddm[<?php echo $name; ?>]" />
                ...
                <div style="margin: 4px 0 4px 0;">
                    <select name="faddm[<?php echo $name; ?>]">
                        ...
                        foreach ($opts as $opt)
                        {
                            ...
                            // show option
                            echo '<option value="'.cnHtmlSpecialChars($lr[0]).'" '.(($lr[0] === $item['#value']) ? 'selected' : '').'>'.cnHtmlSpecialChars($r).'</option>';
                        }
            ...
            <?php if ($item['desc']) echo '<div style="font-size: 10px; color: #808080; margin: 0 0 15px 0;">'.cnHtmlSpecialChars($item['desc']).'</div>'; ?>
