 ...
 * @return array
 */
function cn_render_pagination($array, $_url, $page, $per_page, $adjacents = 3, $name_per_page = 'per_page', $name_page = 'page', $class_active = 'current', $class_pagination = 'light-theme simple-pagination pagination')
{
    $arr = array();
    $paging = new PaginationTemplate();
...
function cn_render_pagination_ajax($array, $_url, $page, $per_page, $isCallAjax = true, $adjacents = 3, $name_per_page = 'per_page', $name_page = 'page', $class_active = 'current', $class_pagination = 'light-theme simple-pagination pagination')
{
    $arr = array();
    $paging = new PaginationTemplate();
...
function cn_form_open($fields)
{
    ...
    // Call check session web or admin
    cn_before_digital_signature_admin_or_web();
}

// Since 2.0: Generate CSRF stamp (only for members)
// @Param: type = std (input hidden), a (inline in a)
function cn_snippet_digital_signature($member, $type = 'std')
{
...
function cn_digital_signature_meta($member)
{
    $htmlResult = '';
    // Is not member - is fatal error
    if (is_null($member)) return $htmlResult;
...
function cn_template_class()
{
    ...
    setMemcache('#class', $class = cn_get_template_by('class'));
    return $class;
}
...
function cn_template_resetvip()
{
    ...
    setMemcache('#resetvip', $options_rsvip);
...
function cn_template_uythacrs()
{
    ...
    setMemcache('#uythacrs', $options_uythac_rs);
...
function cn_template_uythacrsvip()
{
    ...
    setMemcache('#uythacrsvip', $options_uythac_rsvip);
...
function cn_template_rslimit1()
{
    ...
    setMemcache('#rslimit1', $options_rslimit1);
...
function cn_template_rslimit2()
{
    ...
    setMemcache('#rslimit2', $options_rslimit2);
...
function cn_template_httt()
{
    ...
    setMemcache('#hotro_tanthu', $options_httt);
    ...
    if ($resultlistChracter) {
        foreach ($resultlistChracter as $key => $item) {
            if ((($item['TimeThuePoint'] + 86400) <= ctime()) && $item['IsThuePoint']) {
                ...
                $_arr_cls = separateString($arr_cls);
