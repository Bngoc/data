...
if (!$vbulletin->GPC['calendarid']) { // Determine the first calendar we have canview access to for the default calendar
    if ($vbulletin->GPC['eventid']) { // get calendarid for this event
...
if ($_REQUEST['do'] == 'displayweek') {
    ...
    if ($vbulletin->GPC['week']) {
        ...
        while (true) {
            ...
            if ($nextweek > $todaystamp) { // current week was last week so show that week!!
                break;
            } else {
                $offset += 7;
            }
...
if ($_REQUEST['do'] == 'getday' OR $_REQUEST['do'] == 'getinfo') {
    ...
    if ($eventinfo['eventid']) {
        ...
        if ($calendarinfo['showbirthdays']) {  // Load the birthdays for today
...
if ($_POST['do'] == 'update') {
    ...
    if (!$eventinfo['eventid']) { // No Eventid == Insert Event
...
if ($_POST['do'] == 'dostuff') {
    ...
    if (!empty($ids)) {
        if ($vbulletin->GPC['what'] == 'delete') {
            ...
            if (!empty($reminders["{$vbulletin->GPC['what']}"])) { # make sure the supplied integer is a valid one
                $db->query_write("
