        ...
        if ($getval !== null) {
            switch ($val) {
                ...
                case INTEGER:
                    ...
                    break;
                case DATETIME:
                    $out[$key] = strtotime($getval);
                    break;
                case FLOAT:
                    $out[$key] = (float)$getval;
                    break;
                case TOGGLE:
                    if ($getval == 'on') {
                        $out[$key] = 1;
...
handle_attach($action, $attributes, $content, $params, $node_object)
{
    ...
    if ($fr_platform == 'vb40') {
        ...
        if ($attachmentinfo['postid'] == 0) {    // Attachment that is in progress but hasn't been finalized
            if ($vbulletin->userinfo['userid'] != $attachmentinfo['userid'] AND !can_moderate($attachmentinfo['forumid'], 'caneditposts')) {    // Person viewing did not upload it
                return '';
            }
