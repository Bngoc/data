...
do_online()
{
    ...
    while ($users = $db->fetch_array($allusers)) {
        if ($users['userid']) { // Reg'd Member
            ...
            $key = $users['userid'];
            if ($key == $vbulletin->userinfo['userid']) { // in case this is the first view for the user, fake it that show up to themself
                $foundviewer = true;
            }
