    ...
    switch ($method) {
        ...
        case 'copy':
            ...
            while ($post = $db->fetch_array($posts)) {
                ...
                if (!$done_firstpost) {
                    if (!$threadinfo['visible']) {    // Insert Moderation Record
                        $db->query_write("
                ...
                if (!$newpostid) { // the saving of the post failed, skip it
                    continue;
                }
...
if ($_REQUEST['do'] == 'managepost') {
    if ($postinfo['postid'] == $threadinfo['firstpostid']) {    // first post
...
if ($_POST['do'] == 'updatethread') {
    ...
    if ($threadinfo['visible'] == 2) {    // Editing a deleted thread
        if ($vbulletin->GPC['threadstatus'] == 1 AND can_moderate($threadinfo['forumid'], 'candeleteposts')) { // undelete
            undelete_thread($threadinfo['threadid'], $foruminfo['countposts']);
            $threaddeleted = -1;
        } else if ($vbulletin->GPC['threadstatus'] == 2 AND can_moderate($threadinfo['forumid'], 'canremoveposts')) { // remove
            $threadman =& datamanager_init('Thread', $vbulletin, ERRTYPE_STANDARD, 'threadpost');
            $threadman->set_existing($threadinfo);
    ...
    } else {    // Editing a non deleted thread
...
if ($_POST['do'] == 'domergethread') {
    ...
    // sort out polls
    if ($mergethreadinfo['pollid'] != 0) { // merge thread has poll ...
        if ($threadinfo['pollid'] == 0) { // ... and original thread doesn't
            $threadman->set('pollid', $mergethreadinfo['pollid']);
            $threadcache["$mergethreadinfo[threadid]"]['pollid'] = 0;
        } else { // ... and original does
