...
if ($_POST['do'] == 'dosearch') {
    ...
    if ($vbulletin->GPC['exactmatch']) {
        ...
        switch ($vbulletin->GPC['searchwhere']) {
            case 0:
                $sql .= "text = '" . $db->escape_string($vbulletin->GPC['searchstring']) . "'";
                break;
            case 1:
                $sql .= "varname = '" . $db->escape_string($vbulletin->GPC['searchstring']) . "'";
                break;
            case 10:
                $sql .= "(text = '" . $db->escape_string($vbulletin->GPC['searchstring']) . "' OR $sql varname = '" . $db->escape_string($vbulletin->GPC['searchstring']) . "')";
                break;
            default:
                $sql .= '';
        }
        ...
        switch ($vbulletin->GPC['searchwhere']) {
            case 0:
                $sql = fetch_field_like_sql($vbulletin->GPC['searchstring'], 'text', false, $vbulletin->GPC['casesensitive']);
                break;
            case 1:
                $sql = fetch_field_like_sql($vbulletin->GPC['searchstring'], 'varname', true, $vbulletin->GPC['casesensitive']);
                break;
            case 10:
                $sql = '(' . fetch_field_like_sql($vbulletin->GPC['searchstring'], 'text', false, $vbulletin->GPC['casesensitive']) . ' OR ' . fetch_field_like_sql($vbulletin->GPC['searchstring'], 'varname', true, $vbulletin->GPC['casesensitive']) . ')';
                break;
            default:
                $sql = '';
        }
...
if ($_POST['do'] == 'doreplace') {
    ...
    while ($phrase = $db->fetch_array($phrases)) {
        ...
        if ($phrase['languageid'] == $vbulletin->GPC['languageid']) { // update
            $db->query_write("
