...
if ($_POST['do'] == 'update') {
    ...
    if ($newminute["-1"]) { // its run every minute so lets just ignore every other entry
        $newminute = array(0 => -1);
    } else {
...
if ($_REQUEST['do'] == 'modify') {
    ...
    <script type="text/javascript">
        ...
        function js_cron_jump(cronid) {
            ...
            switch (task) {
                case 'edit':
                    window.location = "cronadmin.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=edit&cronid=" + cronid;
                    break;
                case 'kill':
                    window.location = "cronadmin.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=remove&cronid=" + cronid;
                    break;
                case 'switchactive':
                    window.location = "cronadmin.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=switchactive&cronid=" + cronid + "&hash=<?php echo CP_SESSIONHASH; ?>";
                    break;
                default:
                    return false;
                    break;
