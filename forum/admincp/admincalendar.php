...
if ($_REQUEST['do'] == 'addcustom') {
    ...
    if ($vbulletin->GPC['calendarcustomfieldid']) { // edit
    ...
    } else if ($vbulletin->GPC['calendarid']) { // Add new
...
if ($_POST['do'] == 'doaddcustom') {
    ...
    } else if ($vbulletin->GPC['calendarid']) { // Add new Entry
        /*insert query*/
        $db->query_write("
...
if ($_POST['do'] == 'updatemod') {
    ...
    if ($calendarinfo['calendarid'] AND ($userinfo['userid'] OR $vbulletin->GPC['moderatorid'])) { // no errors
        ...
        if ($vbulletin->GPC['moderatorid']) { // update
...
if ($_REQUEST['do'] == 'modifyholiday') {
    ...
    <script type="text/javascript">
        function js_holiday_jump(holidayid, obj) {
            ...
            switch (task) {
                case 'edit':
                    window.location = "admincalendar.php?s=<?php echo $vbulletin->session->vars['sessionhash']; ?>&do=updateholiday&holidayid=" + holidayid;
                    break;
                case 'kill':
                    window.location = "admincalendar.php?s=<?php echo $vbulletin->session->vars['sessionhash']; ?>&do=removeholiday&holidayid=" + holidayid;
                    break;
                default:
                    return false;
                    break;
