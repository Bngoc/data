...
if ($_REQUEST['do'] == 'modify') {
    ...
    <script type="text/javascript">
        function js_usergroup_jump(id, obj) {
            ...
            switch (task) {
                case 'editlevel':
                    window.location = "admininfraction.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=editlevel&infractionlevelid=" + id;
                    break;
                case 'killlevel':
                    window.location = "admininfraction.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=removelevel&infractionlevelid=" + id;
                    break;
                case 'editgroup':
                    window.location = "admininfraction.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=editgroup&infractiongroupid=" + id;
                    break;
                case 'killgroup':
                    window.location = "admininfraction.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=removegroup&infractiongroupid=" + id;
                    break;
                case 'editban':
                    window.location = "admininfraction.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=editbangroup&infractionbanid=" + id;
                    break;
                case 'killban':
                    window.location = "admininfraction.php?<?php echo $vbulletin->session->vars['sessionurl_js']; ?>do=removebangroup&infractionbanid=" + id;
                    break;
                default:
                    return false;
                    break;
    ...
    while ($infraction = $db->fetch_array($infractions)) {
        switch ($infraction['period']) {
            case 'D_1':
                $period = construct_phrase($vbphrase['x_days'], 1);
                break;
            case 'D_2':
                $period = construct_phrase($vbphrase['x_days'], 2);
                break;
            case 'D_3':
                $period = construct_phrase($vbphrase['x_days'], 3);
                break;
            case 'D_4':
                $period = construct_phrase($vbphrase['x_days'], 4);
                break;
            case 'D_5':
                $period = construct_phrase($vbphrase['x_days'], 5);
                break;
            case 'D_6':
                $period = construct_phrase($vbphrase['x_days'], 6);
                break;
            case 'D_7':
                $period = construct_phrase($vbphrase['x_days'], 7);
                break;
            case 'D_10':
                $period = construct_phrase($vbphrase['x_days'], 10);
                break;
            case 'D_14':
                $period = construct_phrase($vbphrase['x_weeks'], 2);
                break;
            case 'D_21':
                $period = construct_phrase($vbphrase['x_weeks'], 3);
                break;
            case 'M_1':
                $period = construct_phrase($vbphrase['x_months'], 1);
                break;
            case 'M_2':
                $period = construct_phrase($vbphrase['x_months'], 2);
                break;
            case 'M_3':
                $period = construct_phrase($vbphrase['x_months'], 3);
                break;
            case 'M_4':
                $period = construct_phrase($vbphrase['x_months'], 4);
                break;
            case 'M_5':
                $period = construct_phrase($vbphrase['x_months'], 5);
                break;
            case 'M_6':
...
if ($_REQUEST['do'] == 'dolist') {
    ...
    switch ($vbulletin->GPC['orderby']) {
        ...
        case 'expires':
            ...
            break;
        case 'username':
            $orderby = 'post.username';
            break;
        case 'leftby_username':
            $orderby = 'leftby_username';
            break;
        default:
