...
?>
<table cellpadding="0" cellspacing="0" border="0" width="<?php echo $OUTERTABLEWIDTH; ?>" align="center">
    <tr valign="top">
        <td>
    ...
    if ($_POST['do'] == 'status') {
        ...
        if ($vbulletin->GPC['userid']) { // already existing entry
    ...
    if ($_REQUEST['do'] == 'adjust') {
        ...
        if ($vbulletin->GPC['subscriptionlogid']) { // already exists
            $sub = $db->query_first("
        ...
        <script type="text/javascript">
            function js_forum_jump(sid) {
                ...
                if (action != '') {
                    switch (action) {
                        case 'edit':
                            page = "subscriptions.php?do=edit&subscriptionid=";
                            break;
                        case 'remove':
                            page = "subscriptions.php?do=remove&subscriptionid=";
                            break;
                        case 'view':
                            page = "subscriptions.php?do=find&status=1&subscriptionid=";
                            break;
                        case 'addu':
                            page = "subscriptions.php?do=adjust&subscriptionid=";
                            break;
                    }
        ...
        <script type="text/javascript">
            function js_forum_jump(pid) {
                ...
                if (action != '') {
                    switch (action) {
                        case 'edit':
                            page = "subscriptions.php?do=apiedit&paymentapiid=";
                            break;
                        case 'remove':
                            page = "subscriptions.php?do=apirem&paymentapiid=";
                            break;
                    }
                    document.cpform.reset();
