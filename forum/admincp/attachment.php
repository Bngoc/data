...
if ($_REQUEST['do'] == 'domoveattachment') {
    ...
    while ($attachment = $db->fetch_array($attachments)) {
        $vbulletin->GPC['count']++;
        $attacherror = false;
        if ($vbulletin->options['attachfile'] == ATTACH_AS_DB) { // Converting FROM mysql TO fs
        ...
        } else { // Converting FROM fs TO mysql
    ...
    <script type="text/javascript">
        ...
        function js_attachment_jump(attachinfo) {
            ...
            if (action != '') {
                switch (action) {
                    case 'edit':
                        page = "attachment.php?do=updatetype&extension=";
                        break;
                    case 'remove':
                        page = "attachment.php?do=removetype&extension=";
                        break;
                    case 'perms':
                        page = "attachmentpermission.php?do=modify&extension=";
...
if ($_REQUEST['do'] == 'updatetype') {
    ...
    if ($vbulletin->GPC['extension']) { // This is an edit
        $type = $db->query_first("
