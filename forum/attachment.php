...
if (isset($_SERVER['HTTP_RANGE'])) {
    ...
    if (intval($matches[1]) < 0) { // its negative so we want to take this value from last byte
...
if ($vbulletin->options['attachfile']) {
    ...
    while (connection_status() == 0 AND $startbyte <= $lastbyte) {    // You can limit bandwidth by decreasing the values in the read size call, they must be equal.
