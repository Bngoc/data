...
(function (A, B) {
    ...
    A("[data-role=page]").live("pageinit", function (C) {
        ...
        A('[name="preview"]', O).unbind().live("vclick", function (Q) {
            A("<input>").attr({type: "hidden", id: "previewinput", name: "preview", value: "1"}).appendTo("form");
        ...
        if (typeof FB !== "undefined") {
            ...
            A('[name="link"]', O).unbind().live("vclick", function (Q) {
                A("<input>").attr({type: "hidden", id: "linkinput", name: "link", value: "1"}).appendTo("form");
                A(this).closest("form").submit();
                return false
            });
            A('[name="nolink"]', O).unbind().live("vclick", function (Q) {
                A("<input>").attr({type: "hidden", id: "nolinkinput", name: "nolink", value: "1"}).appendTo("form");
                A(this).closest("form").submit();
                return false
            });
            function F() {
        ...
        A('[name="draft"]', O).unbind().click(function (Q) {
            A("<input>").attr({type: "hidden", id: "draftinput", name: "draft", value: "1"}).appendTo("form");
            A(this).closest("form").submit();
            return false
        });
        if (A("ul.breadcrumb li", O).length > 1) {
            ...
            A("div.pagetitle", O).click(function () {
                if (!A(O).data("breadcrumbshow")) {
                    ...
                    A(".breadcrumb", O).hide();
                    A(".breadcrumboverlay", O).offset({top: 0, left: 0}).hide();
        ...
        A("button[data-submitname]", O).each(function () {
            A(this).unbind().click(function (S) {
                ...
                A("<input>").attr({type: "hidden", id: R + "input", name: R, value: Q}).appendTo("form");
                A(this).closest("form").submit();
                return false
            })
        });
        ...
        A("button.inlinemod", O).unbind().click(function (R) {
            ...
            A("<input>").attr({type: "hidden", id: Q + "input", name: "do", value: Q}).appendTo("form");
            A(this).closest("form").submit();
            return false
        });
        ...
        function N(R, Q) {
            if (Q) {
                ...
                A("#mq_image_" + R, O).attr("src", BBURL + "/" + IMGDIR_BUTTON + "/multiquote-selected_40b.png")
            } else {
                A("#mq_" + R, O).removeClass("highlight");
                A("#mq_image_" + R, O).attr("src", BBURL + "/" + IMGDIR_BUTTON + "/multiquote_40b.png")
            }
        }
        ...
        function J(V) {
            ...
            A.cookie("vbulletin_multiquote", R.join(","), {path: "/"});
            return false
        }
    ...
    A("[data-role=page]").live("pageshow", function (F) {
        var D = this, C = window.location.href;
...
(function (A, B) {
    A.widget("mobile.imagereg", A.mobile.widget, {
        options: {theme: null}, _create: function () {
            var D = this.element, E = this.options, C = this;
...
(function (A, B) {
    A.widget("mobile.vbpagenav", A.mobile.widget, {
        ...
        pagenumberwidth: 20,
        useqmark: false,
        options: {theme: null, pagenumber: 1, totalpages: 1, address: "", address2: "", anchor: ""},
        _create: function () {
            var E = this.element, I = this.options, C = this;
        ...
        refresh: function () {
            var J = this.options, G = this.element, D = this;
        ...
        swipeleft: function () {
            var I = this.options, G = this.element, E = this;
        ...
        swiperight: function () {
            var H = this.options, E = this.element, C = this;
        ...
        _buildPagenumber: function (F, D) {
            var I = this.options, E = this.element, C = this;
            var H = E.find("div.ui-vbpagenav-pagenumbers");
            var G = A("<a>", {
...
(function (A, C) {
    ...
    A.widget("mobile.listview", A.mobile.listview, {
        _itemApply: function (E, F) {
            ...
            var T = this.options, D = this.element, S = this, X = D.jqmData("dividertheme") || T.dividerTheme, Q = D.jqmData("splittheme"), I = D.jqmData("spliticon"), R = D.children("li"), N = A.support.cssPseudoElement || !A.nodeName(D[0], "ol") ? 0 : 1, V, E, O, W, M, H, F, U;
            if (N) {
            ...
            var J = this.element, K = J.closest(".ui-page"), D = K.jqmData("url"), G = D || K[0][A.expando], E = J.attr("id"), F = this.options, H = "data-" + A.mobile.ns, L = this, I = K.find(":jqmData(role='footer')").jqmData("id");
            if (typeof (B[G]) === "undefined") {
                B[G] = -1
            }
            E = E || ++B[G];
            A(J.find("li>ul, li>ol").toArray().reverse()).each(function (N) {
                var Q = A(this), P = Q.attr("id") || E + "-" + N, S = Q.parent(), U = A(Q.prevAll().toArray().reverse()), U = U.length ? U : A("<span>" + A.trim(S.contents()[0].nodeValue) + "</span>"), R = U.first().text(), M = (D || "") + "&" + A.mobile.subPageUrlKey + "=" + P;
            ...
            var G = this.options, F = this.element, E = this, D = F.children("li");
            D.each(function (I) {
                var H = A(this);
            ...
            A(D).removeAttr("width").removeAttr("height").css({width: "", height: ""});
