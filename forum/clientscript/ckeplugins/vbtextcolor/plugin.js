(function () {
    ...
    CKEDITOR.vbtextcolor = {};

    CKEDITOR.plugins.add('vbtextcolor',
        {
            ...
            init: function (editor) {
                ...
                editor.on('instanceReady', function (editorEvent) {
                    var textcolorButton = getButton(editor, 'button', editor.lang.colorButton.textColorTitle);
                    if (textcolorButton) {
                        var buttonElement = CKEDITOR.document.getById(textcolorButton.id);

                        icon = buttonElement.getChild(0);
                        icon.on('click', CKEDITOR.vbtextcolor.applyLastColor, null, null, 1);
                ...
                CKEDITOR.vbtextcolor.setLastColor = function (color) {
                    CKEDITOR.vbtextcolor.lastColor = color;
                    if (arrow) {
                        if (color) {
                            arrow.setStyle('background-color', color);
                        }
                        else {
                            arrow.removeStyle('background-color');
                        }
                    }
                }

                CKEDITOR.vbtextcolor.applyLastColor = function (event) {
                    if (CKEDITOR.vbtextcolor.lastColor) {
                        CKEDITOR.tools.callFunction(clickFn, CKEDITOR.vbtextcolor.lastColor, 'fore');
                        event.cancel();
                        event.stop();
                        // preventDefault(true) is needed to stop the "Do you want to leave the page" popup from appearing in IE
                ...
                function renderColors(panel, type, colorBoxId) {
                    ...
                    output.push(
                        ...
                        ' role="option" aria-posinset="1" aria-setsize="', total, '"' + (editor.mode != 'wysiwyg' ? ' style="display:none"' : '') + '>' +
