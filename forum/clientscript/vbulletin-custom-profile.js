...
function preview_css_customizations(A) {
    ...
    csspreview.move_preview(A.parentNode);
    csspreview.fetch_preview(A.form)
}
function vB_AJAX_CSSPreview() {
    this.progress = "preview_progress";
    this.preview_area = "usercss"
}
...
function clear_css_customizations() {
    var A, C, B;
    var B = YAHOO.util.Dom.get("usercss_form").getElementsByTagName("input");
    for (A = 0; A < B.length; A++) {
        if (B[A].type == "text" && B[A].value != "") {
            B[A].focus();
            B[A].value = "";
            if (YAHOO.util.Dom.hasClass(B[A], "pickerinput")) {
                console.log("Updating preview for %s", B[A].id);
                update_color_preview(B[A].id)
            }
        }
    }
    var B = YAHOO.util.Dom.get("usercss_form").getElementsByTagName("select");
    for (A = 0; A < B.length; A++) {
        if (B[A].selectedIndex != 0) {
            B[A].focus();
            B[A].selectedIndex = 0
        }
    }
    YAHOO.util.Dom.get("usercss_form").scrollIntoView()
}
