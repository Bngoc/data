...
vB_Popup_Handler.prototype.register = function (D, A, C) {
    ...
    if (B && YAHOO.util.Dom.isAncestor(B, D)) {
        this.menus[D].imgsrc = IMGDIR_MISC + "/black_downward_arrow.png"
    }
    this.menus[D].startup();
    return this.menus[D]
};
vB_Popup_Handler.prototype.register_suggest = function (B, A) {
    this.suggest = new vB_Popup_Suggest(B, A);
    return this.suggest
};
...
function vB_Popup_Menu(C, A, B) {
    ...
    this.imgsrc = IMGDIR_MISC + "/black_downward_arrow.png"
}
...
vB_Popup_Menu.prototype.init_control = function (A) {
    ...
    if (this.controlobj.firstChild && (this.controlobj.firstChild.tagName == "TEXTAREA" || this.controlobj.firstChild.tagName == "INPUT")) {
        ...
        if (!A && !(is_mac && is_ie)) {
            ...
            B.title = "";
            B.alt = "";
            B.setAttribute("style", "vertical-align: middle");
...
vB_Popup_Menu.prototype.set_menu_position = function (A) {
    ...
    YAHOO.util.Dom.setXY(this.menuobj, [this.leftpx, this.toppx])
};
...
function vB_Popup_Suggest(B, A) {
    this.controlkey = B;
    this.noslide = A;
    this.menuname = this.controlkey.split(".")[0] + "_menu";
    this.startup()
}
vB_Popup_Suggest.prototype.init_menu = vB_Popup_Menu.prototype.init_menu;
vB_Popup_Suggest.prototype.set_menu_position = vB_Popup_Menu.prototype.set_menu_position;
vB_Popup_Suggest.prototype.startup = function () {
    this.init_control(this.noimage);
    if (fetch_object(this.menuname)) {
        this.init_menu()
    }
...
vB_Popup_Suggest.prototype.init_control = function (A) {
    this.controlobj = fetch_object(this.controlkey);
    this.controlobj.state = false;
    this.controlobj.unselectable = true;
    YAHOO.util.Event.addListener(this.controlobj, "click", this.controlobj_onclick, this, true)
};
vB_Popup_Suggest.prototype.controlobj_onclick = function () {
    this.hide()
};
vB_Popup_Suggest.prototype.init_menu_contents = function () {
    var E = new Array("td", "li");
    for (var D = 0; D < E.length; D++) {
        var H = fetch_tags(this.menuobj, E[D]);
        for (var F = 0; F < H.length; F++) {
            if (H[F].className == "vbmenu_option" || H[F].className == "vbmenu_hilite") {
                if (H[F].title && H[F].title == "nohilite") {
                    H[F].title = ""
                } else {
                    H[F].controlkey = this.controlkey;
...
vB_Popup_Suggest.prototype.show = function (B, A) {
    if (!vBmenu.active) {
        return false
    } else {
        if (!this.menuobj) {
    ...
    vBmenu.suggest = this;
    console.log("vBSuggest :: Show '%s'", this.controlkey);
    this.menuobj.style.display = "";
    if (this.slide_open) {
        this.menuobj.style.clip = "rect(auto, 0px, 0px, auto)"
    }
...
vB_Popup_Suggest.prototype.hide = function (A) {
    if (A && A.button && A.button != 1) {
        return true
    }
    console.log("vBSuggest :: Hide '%s'", this.controlkey);
    this.stop_slide();
    this.menuobj.style.display = "none";
    this.select_handler.show()
};
vB_Popup_Suggest.prototype.slide = function (C, B, A) {
    if (this.direction == "left" && (C < this.menuobj.offsetWidth || B < this.menuobj.offsetHeight)) {
        C += this.intervalX;
        B += this.intervalY;
        this.menuobj.style.clip = "rect(auto, " + C + "px, " + B + "px, auto)";
        this.slidetimer = setTimeout(this.slide_call(C, B, A), 0)
    } else {
        if (this.direction == "right" && (C > 0 || B < this.menuobj.offsetHeight)) {
            C -= this.intervalX;
            B += this.intervalY;
...
vB_Popup_Suggest.prototype.slide_call = function (C, B, A) {
    var D = this;
    return function () {
        return D.slide(C, B, A)
    }
};
vB_Popup_Suggest.prototype.stop_slide = function () {
    clearTimeout(this.slidetimer);
