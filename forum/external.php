...
if (empty($forumchoice)) {    // no access to view selected forums
    exit;
}
...
$postids = array();
while ($thread = $db->fetch_array($threads)) { // fetch the threads
    // remove sessionhash from urls:
    $forumperms = fetch_permissions($thread['forumid']);
    if (!($forumperms & $vbulletin->bf_ugp_forumpermissions['canviewthreads'])) {    // Don't show thread content and attachments
        $thread['message'] = '';
    } else {
...
$output = '';
$headers = array();
if ($vbulletin->GPC['type'] == 'JS') { // javascript output
    $output = "
...
} else if ($vbulletin->GPC['type'] == 'XML') { // XML output
    ...
    $output .= $xml->output();
    unset($xml);
} else if (in_array($vbulletin->GPC['type'], array('RSS', 'RSS1', 'RSS2'))) { // RSS output
    // setup the board title

    if (empty($title)) { // just show board title
        $rsstitle = $vbulletin->options['bbtitle'];
    } else { // show board title plus selection
