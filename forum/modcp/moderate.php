...
if ($_POST['do'] == 'domessages') {
    ...
    foreach ($vbulletin->GPC['messageaction'] AS $vmid => $action) {
        ...
        if ($action == 1) { // validate
            ...
            $dataman->save();
        } else if ($action == -1 AND can_moderate(0, 'candeletevisitormessages')) { // delete
            $dataman->delete();
        }
    }
...
if ($_POST['do'] == 'doevents') {
    ...
    foreach ($vbulletin->GPC['eventaction'] AS $eventid => $action) {
        ...
        if ($action == 1) { // validate
            ...
            $eventdata->save();
        } else if ($action == -1 AND can_moderate_calendar($getcalendarid['calendarid'], 'candeleteevents')) { // delete
...
if ($_REQUEST['do'] == 'posts') {
    ...
    if (!empty($threadids) AND $sql) {
        ...
        while ($thread = $db->fetch_array($threads)) {
            if ($thread['firstpostid'] == 0) { // eek potential for disaster
...
if ($_POST['do'] == 'doattachments') {
    ...
    foreach ($vbulletin->GPC['attachaction'] AS $attachmentid => $action) {
        if ($action == 0) { // no point in checking the permission if they dont want to do anything to the attachment
            continue;
        }
