...
class vB_DataManager_Infraction extends vB_DataManager
{
    ...
    {
        if ($action OR !$points) {    // Don't go forward if this item didn't start out active or doesn't have any points (warning)
            return;
        }
    ...
    {
        if ($this->existing['action'] != 0) {    // Only reset infraction information for an active infraction. Expired and reversed infractions have already done this
            return;
        }
        ...
        if ($userinfo = $this->info['userinfo'] OR ($this->existing['userid'] AND $userinfo = fetch_userinfo($this->existing['userid']))) {    // Decremement infraction counters and remove any points
            $userdata =& datamanager_init('User', $this->registry, ERRTYPE_SILENT);
            $userdata->set_existing($userinfo);
    ...
    function pre_save($doquery = true)
    {
        ...
        if (!$this->fetch_field('action') AND !$this->condition) {    // active infraction
            $this->set('action', 0);
        }
    ...
    function post_save_each($doquery = true)
    {
        ...
        if (!$this->condition) {
            ...
            if ($this->fetch_field('action') == 2 AND $threadid = $this->fetch_field('threadid') AND $threadinfo = fetch_threadinfo($threadid) AND $foruminfo = $this->registry->forumcache["{$threadinfo['forumid']}"] AND $userid = $this->fetch_field('actionuserid')) {    // Reversed
                $infractioninfo = array(
