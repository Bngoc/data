...
class vB_Visitor_Message
{
    ...
    function process_display()
    {
        ...
        if ($this->converse) {
            if ($this->userinfo['userid'] == $this->registry->userinfo['userid']) {    // viewing our own profile
            ...
            } else if ($this->message['postuserid'] AND $this->message['postuserid'] != $this->userinfo['userid']) {    // Not our profile!
                $show['converse'] = true;
        ...
        $message = $this->message;
        if (!$this->converse) {
            if ($this->userinfo['userid'] != $this->registry->userinfo['userid'] OR $this->message['postuserid'] == $this->userinfo['userid']) {    // This forces the inlinemod checks below to only use the moderator permissions
                $userinfo = null;
                $message = null;
            }
        }
