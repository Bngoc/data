...
class vB_Datastore_eAccelerator extends vB_Datastore
{
    ...
    function do_fetch($title, &$unfetched_items)
    {
        ...
        if (($data = eaccelerator_get($ptitle)) === null) { // appears its not there, lets grab the data
...
class vB_Datastore_Memcached extends vB_Datastore
{
    ...
    function fetch($items)
    {
        ...
        if (!empty($unfetched_items)) {
            if ($this->prefix) { // Remove any prefix for datastore call
    ...
    function do_fetch($title, &$unfetched_items)
    {
        ...
        if (($data = $this->memcache->get($ptitle)) === false) { // appears its not there, lets grab the data
            $unfetched_items[] = $title;
            return false;
        }
...
class vB_Datastore_APC extends vB_Datastore
{
    ...
    function do_fetch($title, &$unfetched_items)
    {
        ...
        if (($data = apc_fetch($ptitle)) === false) { // appears its not there, lets grab the data, lock the shared memory and put it in
            $unfetched_items[] = $title;
            return false;
        }
        $this->register($title, $data);
...
class vB_Datastore_XCache extends vB_Datastore
{
    ...
    function do_fetch($title, &$unfetched_items)
    {
        ...
        if (!xcache_isset($ptitle)) { // appears its not there, lets grab the data, lock the shared memory and put it in
            $unfetched_items[] = $title;
            return false;
        }
