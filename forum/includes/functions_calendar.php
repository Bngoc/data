...
function can_moderate_calendar($calendarid = 0, $do = '', $userid = -1)
{
    ...
    if ($calendarid == 0) { // just check to see if the user is a moderator of any calendar
    ...
    } else { // check to see if user is a moderator of specific calendar
...
function cache_birthdays($weekly = 0)
{
    ...
    if ($calendarinfo['showbirthdays']) {
        ...
        if (!$weekly) { // cache birthdays for a single month
...
function fetch_event_criteria($option)
{
    ...
    if ($option['recurring'] == 1) { // Recurring Daily Event
        $desc = construct_phrase($vbphrase['this_event_occurs_every_x_days'], $options[0]);
    } else if ($option['recurring'] == 2) { // Every Weekday
        $desc = $vbphrase['this_event_occurs_every_weekday'];
    } else if ($option['recurring'] == 3) { // Weekly Recurring Event
        if ($options[1] & 1) { // Sunday
            $seldays = $vbphrase['sunday'];
        }
        if ($options[1] & 2) { // Monday
            if ($seldays) {
...
function cache_event_info(&$event, $month, $day, $year, $adjust = 1, $cache = true)
{
    ...
    if ($event['recurring']) { // this is a recurring event
