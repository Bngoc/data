...
function build_new_post($type = 'thread', $foruminfo, $threadinfo, $postinfo, &$post, &$errors)
{
    ...
    if ($vbulletin->userinfo['userid'] != 0) {
        ...
        if (!$threadinfo['issubscribed'] AND $post['emailupdate'] != 9999) { // user is not subscribed to this thread so insert it
            /*insert query*/
        ...
        } else { // User is subscribed, see if they changed the settings for this thread
            if ($post['emailupdate'] == 9999) {    // Remove this subscription, user chose 'No Subscription'
...
function build_thread_rating($rating, $foruminfo, $threadinfo)
{
    ...
    if ($rating >= 1 AND $rating <= 5 AND $foruminfo['allowratings']) {
        if ($vbulletin->userinfo['forumpermissions'][$foruminfo['forumid']] & $vbulletin->bf_ugp_forumpermissions['canthreadrate']) { // see if voting allowed
            $vote = intval($rating);
			...
			")
                ...
                if ($vbulletin->options['votechange']) { // if allowed to change votes
                    if ($vote != $ratingsel['vote']) { // if vote is different to original
            ...
            } else {    // insert new vote, post_save_each ++ the vote count of the thread
                $threadrate =& datamanager_init('ThreadRate', $vbulletin, ERRTYPE_SILENT);
                $threadrate->set_info('thread', $threadinfo);
...
function exec_send_notification($threadid, $userid, $postid)
{
    ...
    while ($touser = $vbulletin->db->fetch_array($useremails)) {
        ...
        if ($touser['emailupdate'] == 4 AND !empty($touser['icq'])) { // instant notification by ICQ
