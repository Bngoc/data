...
class vB_ProfileBlock_VisitorMessaging extends vB_ProfileBlock
{
    ...
    function prepare_output($id = '', $options = array())
    {
        ...
        if ($this->profile->userinfo['userid'] == $this->registry->userinfo['userid']) {
            ...
            if (($this->profile->userinfo['vmunreadcount'] - $readvms) > 0 AND $this->registry->options['globalignore'] != '') { // We still have unread VMs somewhere, and Tachy is enabled
                build_visitor_message_counters($this->profile->userinfo['userid']);
            } else if ($readvms) { // This is more than likely on the second page
                $this->registry->db->query_write("
