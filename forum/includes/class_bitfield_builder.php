...
class vB_Bitfield_Builder
{
    ...
    function build($layout = true, $include_disabled = false)
    {
        ...
        if ($handle = @opendir(DIR . '/includes/xml/')) {
            while (($file = readdir($handle)) !== false) {
                ...
                if ($data !== false) { // no error parsing at least
                    $temp["$matches[1]"] = $data;
                }
            }
            closedir($handle);
        ...
        // products
        foreach ($temp AS $product => $bitfields) { // main group (usergroup, misc, etc)
            foreach ($bitfields AS $title => $permgroup) { // subgroups such as forumpermissions
    ...
    function fetch($file, $layout, $include_disabled = false)
    {
        ...
        if (!($xml = $xmlobj->parse())) {    // xml parser failed
            return false;
        }

        if (!$include_disabled AND $xml['product'] AND $xml['product'] != 'vbulletin' AND empty($vbulletin->products["$xml[product]"])) {    // This product is disabled
            return false;
        }
        ...
        foreach ($xml['group'] AS $bitgroup) {
            if (!isset($tempdata["$bitgroup[name]"])) { // this file as a group with the same name so don't intialise it
