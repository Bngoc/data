...
function construct_forum_bit($parentid, $depth = 0, $subsonly = 0)
{
    ...
    foreach ($vbulletin->iforumcache["$parentid"] AS $forumid) {
        ...
        if (!$subsonly OR ($subsonly AND !empty($forum['subscribeforumid']))) {
            ...
            if (($forum['options'] & $vbulletin->bf_misc_forumoptions['cancontainthreads'])) { // get appropriate suffix for template name
                $tempext = '_post';
            } else {
                $tempext = '_nopost';
            }

            if (!$vbulletin->options['showforumdescription']) { // blank forum description if set to not show
                $forum['description'] = '';
            }
...
function construct_subforum_bit($parentid, $depth = 1, &$list = array())
{
    ...
    foreach ($vbulletin->iforumcache["$parentid"] AS $forumid) {
        $forum = $vbulletin->forumcache["$forumid"];
        $forumperms = $vbulletin->userinfo['forumpermissions']["$forumid"];
        if (!($forumperms & $vbulletin->bf_ugp_forumpermissions['canview']) AND ($vbulletin->forumcache["$forumid"]['showprivate'] == 1 OR (!$vbulletin->forumcache["$forumid"]['showprivate'] AND !$vbulletin->options['showprivateforums']))) { // no permission to view current forum
            continue;
        }
...
function fetch_iconinfo($iconid = 0)
{
    ...
    switch ($iconid) {
        ...
        default:
            if ($vbulletin->iconcache !== null) { // we can get the icon info from the template cache

                DEVDEBUG("returning iconid:$iconid from the template cache");
                return $vbulletin->iconcache["$iconid"];
            } else { // we have to get the icon from a query
