        ...
        if (($has_img_code & BBCODE_HAS_ATTACH) AND preg_match_all('#\[attach(?:=(right|left|config))?\](\d+)\[/attach\]#i', $fulltext, $matches)) {
            ...
            foreach ($matches[2] AS $key => $attachmentid) {
                ...
                if (!empty($this->attachments["$attachmentid"])) {
                    ...
                    if ($attachment['state'] != 'visible' AND $attachment['userid'] != $this->registry->userinfo['userid']) {    // Don't show inline unless the poster is viewing the post (post preview)
                        continue;
                    }
                    ...
                    switch ($attachment['extension']) {
                        ...
                        case 'pdf':
                            ...
                            } else if ($this->registry->userinfo['showimages'] AND ($forceimage OR $this->registry->options['viewattachedimages'] == 3 OR ($this->registry->options['viewattachedimages'] == 2 AND $attachcount == 1)) AND !in_array($attachment['extension'], array('tiff', 'tif', 'psd', 'pdf'))) {    // Display the attachment with no link to bigger image
                                $replace[] = ($fullsize ? '<div class="size_fullsize">' : '') .
                ...
                } else {    // Belongs to another post so we know nothing about it ... or we are not displying images so always show a link
