...
class vB_PaidSubscription
{
    ...
    function fetch_proper_expirydate($regdate, $length, $units)
    {
        ...
        $length = $length + 0;
        $regdate = $regdate + 0;
        if (!is_int($regdate) OR !is_int($length) OR !is_string($units)) { // its not a valid date
            return false;
        }
    ...
    function build_user_subscription($subscriptionid, $subid, $userid, $regdate = 0, $expirydate = 0, $checkperms = true)
    {
        ...
        if ($expirydate_basis) { // active subscription base the value on our current expirydate
        ...
        if ($user['userid'] AND $sub['subscriptionid']) {
            ...
            if (is_array($subscription_forums) AND !empty($subscription_forums)) {
                ...
                foreach ($subscription_forums AS $key => $forumid) {
                    ...
                    } else { //oops! it seems that some of the subscribed forums have been deleted, lets unset it
    ...
    function delete_user_subscription($subscriptionid, $userid, $subid = -1, $adminoption = false)
    {
        ...
        if ($user['userid'] AND $sub['subscriptionid']) {
            ...
            if ($subid != -1 AND is_array($tmp["$subid"])) {
                ...
                if ($new_expires > TIMENOW) {    // new expiration is still after today so just decremement and return
                    $this->registry->db->query_write("
						UPDATE " . TABLE_PREFIX . "subscriptionlog
            ...
            if (in_array($user['displaygroupid'], explode(',', $groups))) { // they're displaying as one of the usergroups in the subscription
            ...
            if (($sub['nusergroupid'] > 0 AND $user['customtitle'] == 0) OR $reset_title) { // they need a default title
                $usergroup = $this->registry->db->query_first_slave("
					SELECT usertitle
					FROM " . TABLE_PREFIX . "usergroup
					WHERE usergroupid = $user[pusergroupid]
				");
                if (empty($usergroup['usertitle'])) { // should be a title based on minposts it seems then
                    $usergroup = $this->registry->db->query_first_slave("
    ...
    function construct_payment($hash, $methodinfo, $timeinfo, $currency, $subinfo, $userinfo)
    {
        if (file_exists(DIR . '/includes/paymentapi/class_' . $methodinfo['classname'] . '.php')) {
            ...
            if (!empty($methodinfo['settings'])) { // need to convert this from a serialized array with types to a single value
