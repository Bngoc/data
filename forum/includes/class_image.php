...
class vB_Image_Magick extends vB_Image_Abstract
{
    ...
    function fetch_im_exec($command, $args, $needoutput = false, $dieongs = true)
    {
        ...
        if ($this->returnvalue OR $exec === null) {    // error was encountered
            if (!empty($output)) {    // command issued by @exec failed
                if (strpos(strtolower(implode(' ', $output)), 'postscript delegate failed') !== false) {
                    $output[] = fetch_error('install_ghostscript_to_resize_pdf');
                }
                $this->error = $output;
            } else if (!empty($php_errormsg)) {    // @exec failed so display error and remove path reveal
                $this->error = array(fetch_error('php_error_x', str_replace($this->registry->options['magickpath'] . '\\', '', $php_errormsg)));
            } else if ($this->returnvalue == -1) {    // @exec failed but we don't have $php_errormsg to tell us why
                $this->error = array(fetch_error('php_error_unspecified_exec'));
            }
            return false;
        } else {
            $this->error = '';
            if (!empty($output)) {    // $output is an array of returned text
                ...
                if (strpos(strtolower(implode(' ', $output)), 'postscript delegate failed') !== false) {    // this is for IM 6.2.4+ which doesn't return false for exec(convert.exe) on .pdf when GS isn't installed
                    $this->error = array(fetch_error('install_ghostscript_to_resize_pdf'));
                }
                return $output;
            } else if (empty($output) AND $needoutput) {    // $output is empty and we expected something back
                return false;
            } else {    // $output is empty and we didn't expect anything back
                return true;
            }
        }
    }
    ...
    {
        $fp = @fopen($filename, 'rb');
        if (($header = @fread($fp, 4)) == '%PDF') {    // this is a PDF so only look at frame 0 to save mucho processing time
            $frame0 = '[0]';
        }
        @fclose($fp);
    ...
    function fetch_converted_image($filename, $output, $imageinfo, $thumbnail = true, $sharpen = true)
    {
        ...
        if ($thumbnail) {
            if ($size) {    // have to use -thumbnail here .. -sample looks BAD for animated gifs
...
#			}
        ...
        if ($zak = $this->fetch_im_exec('convert', $execute)) {
            return $zak;
        } else if ($sharpen AND !empty($this->error[0]) AND strpos($this->error[0], 'image smaller than radius') !== false) {    // try to resize again, but without sharpen
            $this->error = '';
    ...
    function print_image_from_string($string, $moveabout = true)
    {
        ...
        if ($result = $this->fetch_im_exec('convert', $execute)) {
            ...
            if ($filesize = @filesize($tmpname)) {    // this is here because of a stupid Win32 CGI thingymajig. filesize fails but readfile works, go figure
...
class vB_Image_GD extends vB_Image_Abstract
{
    ...
    function annotatettf($image, $letter, $font, $random = true)
    {
        if ($random) {
            ...
            if ($this->regimageoption['randomshape']) {
                if (function_exists('imageantialias')) {    // See http://bugs.php.net/bug.php?id=28147
    ...
    function fetch_image_info($filename)
    {
        ...
        if ($imageinfo = getimagesize($filename)) {
            ...
            if ($this->imageinfo[2] == 'GIF') {    // get scenes
        ...
        if ($validfile = $this->is_valid_thumbnail_extension(file_extension($filename)) AND $imageinfo = $this->fetch_image_info($location)) {
            ...
            if ($this->fetch_imagetype_from_extension(file_extension($filename)) != $imageinfo[2]) {
                ...
                if (function_exists('memory_get_usage') AND $memory_limit = @ini_get('memory_limit') AND $memory_limit != -1) {
                    ...
                    if ($freemem > 0 AND $tmemory > $freemem AND $tmemory <= ($memorylimit * 3)) {    // attempt to increase memory within reason, no more than triple
