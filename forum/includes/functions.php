...
function fetch_language_fields_sql($addtable = true)
{
    ...
    foreach ($phrasegroups AS $group) {
        ...
        if ($group == 'reputationlevel' AND !$vbulletin->options['reputationenable'] AND VB_AREA == 'Forum') {    // Don't load reputation phrases if reputation is disabled
            continue;
        }
        $sql .= ",
...
function fetch_userinfo(&$userid, $option = 0, $languageid = 0, $nocache = 0, $querymaster = false)
{
    ...
    if ($option & FETCH_USERINFO_LOCATION) { // Process Location info for this user
...
function vbsetcookie($name, $value = '', $permanent = true, $allowsecure = true, $httponly = false)
{
    ...
    if (!headers_sent($filename, $linenum)) { // consider showing an error message if they're not sent using above variables?
    ...
    } else if (empty($vbulletin->db->explain) AND !VB_API) { // VBIV-12815 Pre-set error message for later
        ...
        } else { // If we cannot pre-set, at least display an error.
    ...
    if (!($vbulletin->usergroupcache["$usergroupid"]['genericoptions'] & $vbulletin->bf_ugp_genericoptions['isnotbannedgroup'])) {
        ...
        if (!empty($bannedgroups)) {
            ...
            if ($vbulletin->db->num_rows($infractions)) {
                ...
                if (!$foundbanned) {    // This is when we will be "unbanned"
...
function standard_redirect($message = '', $forceredirect = false, $bypasswhitelist = false)
{
    ...
    if ($vbulletin->url AND !$bypasswhitelist AND !$vbulletin->options['redirect_whitelist_disable']) {
        ...
        if ($urlinfo = @parse_url($vbulletin->url)) {
            if (!$urlinfo['scheme']) {    // url is made full in exec_header_redirect which stops a url from being redirected to, say "www.php.net" (no http://)
                $foundurl = true;
            } else {
                $whitelist = array();
...
function cache_permissions(&$user, $getforumpermissions = true, $resetaccess = false, $calperms = true)
{
    ...
    if ($resetaccess AND !$reset) {    // Reset the accesscache array for permissions following access mask update. Only allows one reset.
        $accesscache = array();
        $reset = true;
    }
    ...
    if ($USERGROUPID == 0) { // set a default usergroupid if none is set
        $USERGROUPID = 1;
    }
    ...
    if (defined('SKIP_SESSIONCREATE') AND $user['userid'] == $vbulletin->userinfo['userid'] AND !($user['permissions']['forumpermissions'] & $vbulletin->bf_ugp_forumpermissions['canview'])) {    // grant canview for usergroup if session skipping is defined.
...
function can_moderate($forumid = 0, $do = '', $userid = -1, $usergroupids = '')
{
    ...
    if ($forumid == 0) { // just check to see if the user is a moderator of any forum
        if (isset($imodcache)) { // loop through imodcache to find user
            ...
            return false;
        } else { // imodcache is not set - do a query
    ...
    } else { // check to see if user is a moderator of specific forum
        $getmodperms = fetch_moderator_permissions($forumid, $userid, $issupermod);

        if (($getmodperms['permissions'] OR $getmodperms['permissions2']) AND empty($do)) { // check if user is a mod - no specific permission required
            return true;
        } else { // check if user is a mod and has permissions to '$do'
...
function fetch_stylevars(&$style, $userinfo)
{
    ...
    } else if ($vbulletin->options['remoteyui'] == 1) {    // Yahoo CDN
        $show['remoteyui'] = true;
        $show['remoteyuicombo'] = true;
    ...
    } else if ($vbulletin->options['remoteyui'] == 2) {    // Google CDN (Doesn't support Combo)
        $show['remoteyui'] = true;
        $show['remoteyuicombo'] = false;
    ...
    } else if ($vbulletin->options['remotejquery'] == 1) {    // Google CDN
        $show['remotejquery'] = true;
    ...
    } else if ($vbulletin->options['remotejquery'] == 2) {    // jQuery CDN
        $show['remotejquery'] = true;
        vB_Template_Runtime::addStyleVar('jquerymain', REQ_PROTOCOL . '://code.jquery.com/jquery-' . JQUERY_VERSION . '.min.js');
        //vB_Template_Runtime::addStyleVar('jquerymobilemain', REQ_PROTOCOL . '://code.jquery.com/mobile/' . JQUERY_MOBILE_VERSION . '/jquery.mobile-' . JQUERY_MOBILE_VERSION . '.min.js');
    } else if ($vbulletin->options['remotejquery'] == 3) {    // Microsoft CDN
        $show['remotejquery'] = true;
...
function construct_style_options($styleid = -1, $depthmark = '', $init = true, $quickchooser = false, &$stylecount = 0)
{
    ...
    foreach ($cache AS $x) {
        foreach ($x AS $style) {
            if (!$vbulletin->options['allowchangestyles']) {    // Only allow the default style and any mobile styles.
...
function exec_shut_down()
{
    ...
    if (!$vbulletin->options['bbactive'] AND !($vbulletin->userinfo['permissions']['adminpermissions'] & $vbulletin->bf_ugp_adminpermissions['cancontrolpanel'])) { // Forum is disabled and this is not someone with admin access
