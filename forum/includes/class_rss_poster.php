...
class vB_RSS_Poster
{
    ...
    function fetch_xml($url)
    {
        ...
        if ($xml_string === false OR empty($xml_string['body'])) { // error returned
            if (VB_AREA == 'AdminCP') {
    ...
    function parse_xml($target_encoding = false, $ncrencode = false, $override_encoding = false, $escape_html = false)
    {
        ...
        if ($this->xml_object->parse_xml()) {
            ...
            if (preg_match("#^http://www.w3.org/2005/atom$#i", $this->xml_array['xmlns'])) {
                ...
                $this->feedtype = 'rss';
            } else { // Rather than just continue with an unknown type, show an error
        ...
        switch ($this->feedtype) {
            ...
            case 'rss': {
                ...
                switch ($field) {
                    ...
                    case 'description': {    // this can be handled by the default case
                        return get_item_value($item['description']);
                    }
                        break;

                    case 'title': {    // this can be handled by the default case
                        return get_item_value($item['title']);
                    }
                        break;
