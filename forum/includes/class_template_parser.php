...
class vB_SpecialtyTagParser
{
    ...
    private function state_tag()
    {
        ...
        if ($previous_character === '{') { // we can't walk forward in the start state since for curlys we need to know what we're dealing with
...
class vB_Template_Parser
{
    ...
    {
        if ($this->scanner->token === vbTemplateParser::SIMPLE_VAR) { // hack because we allow someone not to specify {var }
            $token = array(
                'tag_name' => 'var',
                'type' => 'curly',
            );
