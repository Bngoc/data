                ...
                if (in_coventry($fromuser['userid'], true) AND $user['userid'] != $fromuser['userid']) {
                    ...
                    if ($user['permissions'] < 1) {
                        ...
                        if ($user['pmtotal'] >= $user['permissions']['pmquota'] AND !$this->overridequota) {
                            ...
                            if (!($user['options'] & $this->registry->bf_misc_useroptions['pmboxwarning']) AND $user['permissions']['pmquota'] AND ((($user['pmtotal'] + 1) / $user['permissions']['pmquota']) >= .9)) {    // Send email about box being almost full
        ...
        if (!$this->condition) {
            if (is_array($this->info['recipients'])) {
                ...
                foreach ($this->info['recipients'] AS $userid => $user) {
                    ...
                    if (!empty($this->info['pmwarning']["$user[userid]"]) AND !($user['options'] & $this->registry->bf_misc_useroptions['pmboxwarning'])) {    // email user about pm box nearly being full
