...
if ($_POST['do'] == 'confirmemptyfolder') { // confirmation page
...
if ($_REQUEST['do'] == 'dopmreceipt') {
    ...
    if ($vbulletin->GPC['type'] == 'img') {
        ...
        <head><title><?php echo $vbulletin->options['bbtitle']; ?></title>
            <style type="text/css"><?php echo $style['css']; ?></style>
        </head>
        <body>
            ...
            foreach ($pms AS $pmid => $pm) {
                if (in_array($pm['fromuserid'], $ignoreusers)) {
                    ...
                    if (!VB_API) {
                        ...
                        if ($clc == 1) {    // Only one username ? we only send userinfo
