...
if ($_REQUEST['do'] == 'addlist') {
    ...
    if ($vbulletin->GPC['userlist'] == 'buddy' OR $vbulletin->GPC['userlist'] == 'friend') {
        ...
        if ($vbulletin->GPC['userlist'] == 'friend') {
            if ($ouruser['friend'] == 'pending' OR $ouruser['friend'] == 'denied') {    // We are pending friends
                print_standard_redirect(array('redirect_friendspending', $userinfo['username']), true, true);
            } else if ($ouruser['friend'] == 'yes') {    // We are already friends
                print_standard_redirect(array('redirect_friendsalready', $userinfo['username']), true, true);
            } else if ($vbulletin->GPC['userid'] == $vbulletin->userinfo['userid']) { // You can't be friends with yourself
...
if ($_POST['do'] == 'doaddlist') {
    ...
    if ($vbulletin->GPC['userlist'] == 'buddy') { // if an entry exists already then we're fine
        if (empty($ouruser)) {
            $db->query_write("
				INSERT IGNORE INTO " . TABLE_PREFIX . "userlist
					(userid, relationid, type, friend)
...
if ($_POST['do'] == 'updatelist') {
    ...
    if ($vbulletin->GPC['userlist'] == 'buddy') { // FRIENDS LIST, BUDDY LIST or PENDING FRIENDS
        foreach ($clean_lists AS $type => $val) {
            switch ($type) {
                case 'friend_original': { // someone who is currently my friend, if they are missing then I dont want to be their friend
                ...
                case 'buddy_original': { // someone who is simply just a buddy or has denied me friend access, if they are missing from the buddy then they were deleted
        ...
        if (!empty($vbulletin->GPC['username'])) { // friend request
			...
			") AND (!$vbulletin->GPC_exists['makefriends'] OR $userinfo['userid'] != $vbulletin->userinfo['userid'])
                ...
                (
                    ...
                    if (empty($userinfo['friend'])) { // we're not already a buddy so re-add it
    ...
    } else if ($vbulletin->GPC['userlist'] == 'incoming') { // APPROVAL OF NEW FRIENDS
	...
	*/
    ...
    if (!empty($add)) { // It is possible to have multiple ADD calls when you're approving people. Just in case you think it should be only one value.
        $addvalues = array();
        ...
        if (empty($add['approvals'])) { // We need to know a previous state.
            ...
            if (!empty($add['friend'])) {
                ...
                foreach ($add['friend'] AS $userid => $userinfo) {
                    ...
                    if (isset($pendingcache['buddy']["$userid"]) AND $pendingcache['buddy']["$userid"]['friend'] == 'denied') { // If they were denied last time you must have changed your mind, remove the block so its just a buddy
            ...
            } else if (!empty($add['buddy'])) { // We only want a record if one doesn't exist
            ...
            } else if (!empty($add['ignore'])) { // Adding someone to the ignore again is fine
        ...
        if (!empty($add['approvals'])) { // Approving a bunch of users, make sure we get an entry too
            $updatecount_sql = $db->query_read("
				SELECT userid
				FROM " . TABLE_PREFIX . "userlist
				WHERE type = 'buddy'
...
if ($_POST['do'] == 'leavegroup') {
    ...
    if (empty($membergroups)) { // check they have membergroups
        eval(standard_error(fetch_error('usergroup_cantleave_notmember')));
    } else if (!in_array($vbulletin->GPC['usergroupid'], $membergroups)) { // check they are a member before leaving
        eval(standard_error(fetch_error('usergroup_cantleave_notmember')));
    } else {
...
if ($_POST['do'] == 'joingroup') {
    ...
    if (in_array($usergroupid, $membergroups)) {
        ...
        if ($vbulletin->usergroupcache["$usergroupid"]['ispublicgroup']) {
            ...
            if ($db->num_rows($leaders)) { // group is moderated: show join request page
                $clc = 0;
                $groupleaders = array();
