...
if ($_POST['do'] == 'message') {
    ...
    if (isset($_POST['fromquickcomment'])) {    // Don't update Who's Online for Quick Comments since it will get stuck on that until the user goes somewhere else
...
if ($vbulletin->GPC['vmid']) {
    ...
    if (!fetch_visitor_message_perm('caneditvisitormessages', $messageinfo, $messageinfo) AND !($_REQUEST['do'] == 'report' OR $_REQUEST['do'] == 'sendemail')) {        // yes, two instances of $messageinfo in the previous line
        print_no_permission();
    }
