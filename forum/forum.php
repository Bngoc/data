...
if ($vbulletin->options['showevents']) {
    ...
    if (!empty($events)) {
        ...
        foreach ($events AS $index => $value) {
            ...
            if (!$vbulletin->options['showeventtype']) {    // Group by Event // $index = $eventid
                $eventinfo = $eventstore["$index"];
                if (empty($eventinfo['recurring']) AND empty($eventinfo['singleday'])) {    // ranged event -- show it from its real start and real end date (vbgmdate)
