...
if ($postinfo['postid']) {
    ...
    if ($infractioninfo['postid']) {    // this infraction belongs to a post
...
if ($_REQUEST['do'] == 'view') {
    ...
    } else if ($infractioninfo['action'] == 1) {    // Expired
        $show['expired'] = true;
...
if ($_POST['do'] == 'update') {
    ...
    if (!$vbulletin->GPC['infractionlevelid'] AND $vbulletin->userinfo['permissions']['genericpermissions'] & $vbulletin->bf_ugp_genericpermissions['cangivearbinfraction']) {    // custom infraction
        if (empty($vbulletin->GPC['customreason']) OR (!$vbulletin->GPC['expires'] AND $vbulletin->GPC['period'] != 'N')) {
            ...
            switch ($vbulletin->GPC['period']) {
                case 'D':
                    $expires = mktime(date('H'), date('i'), date('s'), date('m'), date('d') + $vbulletin->GPC['expires'], date('y'));
                    break;
                case 'M':
                    $expires = mktime(date('H'), date('i'), date('s'), date('m') + $vbulletin->GPC['expires'], date('d'), date('y'));
                    break;
                case 'N':
                    $expires = 0;
                    break;
            ...
            if (isset($infcache["$infractionlevel[infractionlevelid]"]['expires'])) {
                if ($infcache["$infractionlevel[infractionlevelid]"]['expires'] == 0) {
                    ...
                    switch ($infractionlevel['period']) {
                        ...
                        case 'H':
                        default:
                            $expires = $expiretime + mktime(date('H') + $infractionlevel['expires'], date('i'), date('s'), date('m'), date('d'), date('y'));
                            break;
                    }
...
if ($_REQUEST['do'] == 'report') {
            ...
            if ($infraction['extend'] AND isset($infcache["$infraction[infractionlevelid]"]['expires'])) {
                if ($infcache["$infraction[infractionlevelid]"]['expires'] == 0) {
                    ...
                    switch ($infraction['period']) {
                        case 'D':
                            $expiretime += mktime(date('H'), date('i'), date('s'), date('m'), date('d') + $infraction['expires'], date('y'));
                            break;
                        case 'M':
                            $expiretime += mktime(date('H'), date('i'), date('s'), date('m') + $infraction['expires'], date('d'), date('y'));
                            break;
                        case 'N':
                            $expiretime = 0;
                            break;
    ...
    } else if ($showemail) {    // Send email to user
        $show['email'] = true;
    }
