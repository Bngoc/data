...
switch ($_POST['do']) {
    ...
    case 'spamconfirm':
    case 'dodeletespam': { // thse can be either posts OR threads
        $vbulletin->input->clean_array_gpc('p', array(
            'type' => TYPE_STR,
        ));
...
if ($_POST['do'] == 'stick' OR $_POST['do'] == 'unstick') {
    ...
    if (!empty($threadarray)) {
        ...
        foreach (array_keys($threadarray) AS $threadid) {
            if (!in_array($threadid, $redirect)) {    // Don't add log entry for (un)sticking a redirect
                $modlog[] = array(
                    'userid' => & $vbulletin->userinfo['userid'],
                    'forumid' => & $threadarray["$threadid"]['forumid'],
                    'threadid' => $threadid,
...
if ($_POST['do'] == 'deletethread' OR $_POST['do'] == 'spamthread') {
    ...
    switch ($_POST['do']) {
        ...
        default: {
            if ($threadcount == $redirectcount) {    // selected all redirects so delet-o-matic them
...
if ($_POST['do'] == 'spamconfirm' OR $_POST['do'] == 'dodeletespam') {
    ...
    if ($vbulletin->GPC['type'] == 'thread') { // threads
        $threadarray = array();
        $threads = $db->query_read_slave("
			SELECT threadid, open, visible, forumid, title, prefixid, postuserid
			FROM " . TABLE_PREFIX . "thread
        ...
        while ($thread = $db->fetch_array($threads)) {
            ...
            if ($thread['open'] == 10) {
                if (!can_moderate($thread['forumid'], 'canmanagethreads')) { // No permission to remove redirects.
                    eval(standard_error(fetch_error('you_do_not_have_permission_to_manage_thread_redirects', $thread['prefix_plain_html'] . $thread['title'], $vbulletin->forumcache["$thread[forumid]"]['title'])));
                }
            } else if (!$thread['visible'] AND !can_moderate($thread['forumid'], 'canmoderateposts')) {
                eval(standard_error(fetch_error('you_do_not_have_permission_to_manage_moderated_threads_and_posts')));
...
if ($_POST['do'] == 'dodeletespam') {
        ...
        switch ($vbulletin->GPC['useraction']) {
            case 'ban':
                ...
                while ($current_ban = $db->fetch_array($current_bans)) {
                    ...
                    if ($current_ban['bandate']) { // they already have a ban, check if the current one is being made permanent, continue if its not
    ...
    // report
    if ($vbulletin->GPC['report'] AND !empty($vbulletin->options['vb_antispam_key'])) { // report to Akismet
    ...
    if (!empty($postids)) {
        ...
        while ($post = $db->fetch_array($posts)) {
            ...
            if ($post['firstpostid'] == $post['postid']) {    // deleting a thread so do not decremement the counters of any other posts in this thread
    ...
    foreach ($postarray AS $postid => $post) {
        ...
        if ($vbulletin->GPC['threadid'] == $post['threadid'] AND $post['postid'] == $post['firstpostid']) {    // we've deleted the thread that we activated this action from so we can only return to the forum
            $gotothread = false;
        } else if ($post['postid'] == $postinfo['postid'] AND $physicaldel) {    // we came in via a post, which we have deleted so we have to go back to the thread
    ...
    } else if ($vbulletin->GPC['forumid']) {    // redirect to the forum that we activated from since we hard deleted the thread
...
if ($_POST['do'] == 'approvethread') {
    ...
    while ($thread = $db->fetch_array($threads)) {
        ...
        if ($foruminfo['countposts']) {    // this thread is in a counting forum
    ...
    if (!empty($countingthreads)) {    // Update post count for visible posts
        $userbyuserid = array();
        $posts = $db->query_read_slave("
			SELECT userid
...
if ($_POST['do'] == 'unapprovethread') {
    ...
    while ($thread = $db->fetch_array($threads)) {
        ...
        $foruminfo = fetch_foruminfo($thread['forumid']);
        if ($thread['visible'] AND $foruminfo['countposts']) {    // this thread is visible AND in a counting forum
            $countingthreads[] = $thread['threadid'];
        }
        ...
        if (!empty($threadarray)) {
            ...
            if (!empty($countingthreads)) {
                ...
                while ($post = $db->fetch_array($posts)) {
                    ...
                    if ($foruminfo['countposts'] AND !$destforuminfo['countposts']) {    // Take away a post
                        if (!isset($userbyuserid["$post[userid]"])) {
                            $userbyuserid["$post[userid]"] = -1;
                        } else {
                            $userbyuserid["$post[userid]"]--;
                        }
                    } else if (!$foruminfo['countposts'] AND $destforuminfo['countposts']) {    // Add a post
                        if (!isset($userbyuserid["$post[userid]"])) {
                            $userbyuserid["$post[userid]"] = 1;
                        } else {
                            $userbyuserid["$post[userid]"]++;
...
if ($_POST['do'] == 'domergethreads') {
    ...
    if ($vbulletin->GPC['type'] == 1) {    // Mod cannot create merged hidden thread if they can't moderateposts dest forum
        if (!can_moderate($destforuminfo['forumid'], 'canmoderateposts')) {
            eval(standard_error(fetch_error('you_do_not_have_permission_to_manage_moderated_threads_and_posts_in_destination_forum')));
        }
    } else if ($vbulletin->GPC['type'] == 2) {    // Mod can not create merged deleted thread if they can't deletethreads in dest forum
...
if ($_POST['do'] == 'undeleteposts') {
    ...
    while ($post = $db->fetch_array($posts)) {
        ...
        if ($post['firstpostid'] == $post['postid']) {    // undeleting a thread so need to update the $tinfo for any other posts in this thread
            $firstpost["$post[threadid]"] = true;
        } else if (!empty($firstpost["$post[threadid]"])) {
...
if ($_POST['do'] == 'approveposts') {
    ...
    while ($post = $db->fetch_array($posts)) {
        ...
        if ($post['firstpostid'] == $post['postid']) {    // approving a thread so need to update the $tinfo for any other posts in this thread
            $firstpost["$post[threadid]"] = true;
        } else if (!empty($firstpost["$post[threadid]"])) {
            $postarray["$post[postid]"]['thread_visible'] = 1;
        }
...
if ($_POST['do'] == 'unapproveposts') {
    ...
    while ($post = $db->fetch_array($posts)) {
        ...
        $threadlist["$post[threadid]"] = true;
        $forumlist["$post[forumid]"] = true;
        if ($post['firstpostid'] == $post['postid'] AND $post['thread_visible'] == 1) {    // unapproving a thread so do not decremement the counters of any other posts in this thread
            $firstpost["$post[threadid]"] = true;
        } else if (!empty($firstpost["$post[threadid]"])) {
            $postarray["$post[postid]"]['skippostcount'] = true;
        }
...
if ($_POST['do'] == 'domergeposts') {
    ...
    if ($destpost['threadid'] != $threadinfo['threadid']) {    // retrieve threadinfo for the owner of the first post
    ...
    if (sizeof($errors) > 0) {
        ...
        if ($userid != $destpost['userid'] AND $threadinfo['visible'] == 1 AND $destpost['visible'] == 1 AND $foruminfo['countposts']) {
            if ($userid) {    // need to give this a user a post for now owning the merged post
            ...
            if ($destpost['userid']) {    // need to take a post from this user since they no longer own the merged post
                $user =& datamanager_init('User', $vbulletin, ERRTYPE_STANDARD);
        ...
        foreach ($postarray AS $postid => $post) {
            // Search index maintenance
            vb_Search_Indexcontroller_Queue::indexQueue('vBForum', 'Post', 'delete', $postid);
            if (!empty($deletedthreads["$post[threadid]"])) {    // we already deleted the firstpost of this thread and hence all of its posts so no need to do anything else with this post
                continue;
            }

            $foruminfo = fetch_foruminfo($post['forumid']);

            if ($post['postid'] == $post['firstpostid']) {    // this is a firstpost so check if we can delete this thread or we need to give the thread a new firstpost before we call delete
                ...
                } else { // there are no posts left or we plan to delete them all so mark this thread as deleted now
...
if ($_POST['do'] == 'domoveposts') {
    ...
    if ($vbulletin->GPC['type'] == 0) {    // Move to new thread
    ...
    if ($vbulletin->GPC['type'] == 0) {    // Create a new thread
    ...
    if ($firstpost['dateline'] <= $destthreadinfo['dateline']) {    // destination thread has a new first post (this will always be true for $type == 0)
        if ($firstpost['visible'] != 1) {    // Unhide the new first post since all first posts are visible
            $postman =& datamanager_init('Post', $vbulletin, ERRTYPE_SILENT, 'threadpost');
            ...
            if ($firstpost['firstpostid'] != $firstpost['postid']) {    // We didn't take the thread's first post so remove some records
                if (!$firstpost['visible']) {    // remove new first post's old moderation record
                    $db->query_write("
						DELETE FROM " . TABLE_PREFIX . "moderation
						WHERE primaryid = $firstpost[postid]
							AND type = 'reply'
					");
                } else {    // remove new first post's old deletionlog record
        ...
        if (!$destthreadinfo['visible']) {    // Moderated thread so overwrite moderation record
            $db->query_write("
				REPLACE INTO " . TABLE_PREFIX . "moderation
				(primaryid, type, dateline)
				VALUES
			...
			");
        } else if ($destthreadinfo['visible'] == 2) {    // Deleted thread so overwrite the deletionlog entry
    ...
    foreach ($postarray AS $postid => $post) {
        if ($post['userid'] AND $post['visible'] == 1) {
            ...
            if ($foruminfo['countposts'] AND $post['thread_visible'] == 1 AND (!$destforuminfo['countposts'] OR ($destforuminfo['countposts'] AND $destthreadinfo['visible'] != 1))) {    // Take away a post
                if (!isset($userbyuserid["$post[userid]"])) {
                    $userbyuserid["$post[userid]"] = -1;
                } else {
                    $userbyuserid["$post[userid]"]--;
                }
            } else if ($destforuminfo['countposts'] AND $destthreadinfo['visible'] == 1 AND (!$foruminfo['countposts'] OR ($foruminfo['countposts'] AND $post['thread_visible'] != 1))) {    // Add a post
                if (!isset($userbyuserid["$post[userid]"])) {
                    $userbyuserid["$post[userid]"] = 1;
                } else {
                    $userbyuserid["$post[userid]"]++;
        ...
        if ($post['postid'] == $post['firstpostid']) {    // we moved a first post so thread must be tinkered with
			...
			")
                ...
                if (!$firstleftpost['visible']) {    // new first post is moderated so we must remove it's moderation record
                    $db->query_write("
						DELETE FROM " . TABLE_PREFIX . "moderation
						WHERE primaryid = $firstleftpost[postid]
							AND type = 'reply'
					");
                } else if ($firstleftpost['visible'] == 2) {    // new first post is deleted so we must removed it's deletionlog record
                    $deletiondata =& datamanager_init('Deletionlog_ThreadPost', $vbulletin, ERRTYPE_SILENT, 'deletionlog');
                ...
                if ($firstleftpost['visible'] != 1) {    // post is not visible so we need to set it visible since first posts are always visible
                    $postman =& datamanager_init('Post', $vbulletin, ERRTYPE_SILENT, 'threadpost');
...
if ($_POST['do'] == 'docopyposts') {
    ...
    if ($vbulletin->GPC['type'] == 0) {    // Copy to new thread
        if (empty($vbulletin->GPC['title'])) {
            eval(standard_error(fetch_error('notitle')));
        }
    ...
    while ($post = $db->fetch_array($posts)) {
        ...
        $newfirstpost = false;
        if (!$foundfirstpost) {    // First post!
            if ($post['dateline'] < $destthreadinfo['dateline']) {    // this copied post is the new first post of the thread
                $post['parentid'] = 0;
                if ($post['visible'] != 1) {
                    $post['visible'] = 1;
                }
        ...
        } else if ($postarray["$post[parentid]"] AND $postassoc["$post[parentid]"]) {    // this post's parent was also copied, update it with the new copy
