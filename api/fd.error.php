...
<head>
    ...
    <script type="text/javascript">
        ...
        var developerKey = 'AIzaSyCNdBFLXHcaqEZhoeArbJcpDRyXp0VDSYw';
        // The Client ID obtained from the Google Developers Console.
        var clientId = '770014902535-b8mdqgjbc3beavmjsjmi9bcdg1vjd9bj.apps.googleusercontent.com';
        ...
        function createPicker() {
            ...
            if (pickerAPILoaded && oauthToken) {
                var picker = new google.picker.PickerBuilder().enableFeature(google.picker.Feature.MULTISELECT_ENABLED).addView(docsView).setOAuthToken(oauthToken).setDeveloperKey(developerKey).setCallback(pickerCallback).build();
        ...
        function createPicker_upload(url) {
            ...
            Var
            uploadView = new google.picker.DocsUploadView().setParent(folder_id[0]);
            if (pickerAPILoaded && oauthToken) {
                var picker = new google.picker.PickerBuilder().enableFeature(google.picker.Feature.MULTISELECT_ENABLED).addView(uploadView).setOAuthToken(oauthToken).setDeveloperKey(developerKey).setCallback(pickerCallback_upload).build();
                picker.setVisible(true);
            }
        }
